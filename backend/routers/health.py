...
@router.get(
    "/health/statistics", dependencies=[Depends(require_permission(Permission.HEALTH_READ))]
)
async def get_health_statistics():
    ...
    try:
        ...
        return error(
            "Statistics unavailable — check server logs",
            {
                "total_elements": 0,
                "deleted_elements": 0,
                "active_elements": 0,
...
@router.get(
    "/reports/statistics", dependencies=[Depends(require_permission(Permission.HEALTH_READ))]
)
