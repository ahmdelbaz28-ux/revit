...
class Database:
    ...
    def _init_sqlite(self, db_path: str) -> None:
        ...
        logger.info("SQLite database initialized with performance pragmas at %s", db_path)
    ...
    def _init_postgres(self) -> None:
        ...
        try:
            ...
            logger.error("PostgreSQL mode requires psycopg2. Install it with: pip install psycopg2-binary")
            raise ImportError("PostgreSQL mode requires psycopg2. Install it with: pip install psycopg2-binary")
    ...
    def _transaction(self):
        """Thread-safe transaction context manager for both SQLite and PostgreSQL."""
        if self._is_postgres:
    ...
    def _row_to_project(self, row: Any, device_count: int = 0, connection_count: int = 0) -> dict:
        return {
            "id": row["id"],
            "name": row["name"],
    ...
    def create_project(self, project_data: dict) -> dict:
        ...
        with self._transaction() as cur:
            cur.execute(
                "INSERT INTO projects (id, name, description, author, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    project_data["id"],
                    project_data["name"],
    ...
    def list_projects(
        ...
        projects = [self._row_to_project(row, row["device_count"], row["connection_count"]) for row in rows]
    ...
    def update_project(self, project_id: str, updates: dict) -> dict | None:
        """Update specific fields of a project. Returns updated project or None if not found."""
        set_clauses = []
        values = []
    ...
    def create_device(self, project_id: str, device_data: dict) -> dict:
        ...
        with self._transaction() as cur:
            cur.execute(
                "INSERT INTO devices (id, project_id, type, name, category, x, y, z, rotation, voltage, current, load, properties, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    device_data["id"],
                    project_id,
                    device_data["type"],
    ...
    def list_devices(
        ...
        with self._transaction() as cur:
            ...
            cur.execute(
                "SELECT * FROM devices WHERE project_id = ? ORDER BY " + sort + " " + order + " LIMIT ? OFFSET ?",
    ...
    def update_device(self, project_id: str, device_id: str, updates: dict) -> dict | None:
        ...
        with self._transaction() as cur:
            cur.execute(
                "UPDATE devices SET " + ", ".join(set_clauses) + " WHERE id = ? AND project_id = ?",
                values,
            )  # noqa: S608
    ...
    def create_connection(self, project_id: str, conn_data: dict) -> dict:
        ...
        with self._transaction() as cur:
            ...
            if not cur.fetchone():
                raise ValueError(
                    "Cannot create connection: from_id '%s' does not exist in project '%s'" % (from_id, project_id)
                )
            cur.execute(
                "SELECT id FROM devices WHERE id = ? AND project_id = ?",
                (to_id, project_id),
            ...
            if not cur.fetchone():
                raise ValueError("Cannot create connection: to_id '%s' does not exist in project '%s'" % (to_id, project_id))

            cur.execute(
                "INSERT INTO connections (id, project_id, from_id, to_id, cable_size, length, type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    conn_data["id"],
                    project_id,
                    conn_data["fromId"],
    ...
    def list_connections(
        ...
        with self._transaction() as cur:
            ...
            # sort and order are SAFE here (whitelisted above) — lgtm[py/sql-injection]
            cur.execute(
                "SELECT * FROM connections WHERE project_id = ? ORDER BY " + sort + " " + order + " LIMIT ? OFFSET ?",
                (project_id, limit, offset),
            )  # noqa: S608
            rows = cur.fetchall()
    ...
    def update_connection(self, project_id: str, connection_id: str, updates: dict) -> dict | None:
        ...
        for field, value in updates.items():
            if field in _FIELD_MAP:
                set_parts.append(_FIELD_MAP[field] + " = ?")
                values.append(value)
        ...
        with self._transaction() as cur:
            cur.execute(
                "UPDATE connections SET " + ", ".join(set_parts) + " WHERE id = ? AND project_id = ?",
                values,
            )  # noqa: S608
            if cur.rowcount == 0:
                return None
    ...
    def create_report(self, project_id: str, report_data: dict) -> dict:
        ...
        with self._transaction() as cur:
            cur.execute(
                "INSERT INTO reports (id, project_id, type, name, parameters, status, created_at, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    report_data["id"],
                    project_id,
                    report_data["type"],
    ...
    def list_reports(
        ...
        with self._transaction() as cur:
            ...
            # sort and order are SAFE here (whitelisted above) — lgtm[py/sql-injection]
            cur.execute(
                "SELECT * FROM reports WHERE project_id = ? ORDER BY " + sort + " " + order + " LIMIT ? OFFSET ?",
                (project_id, limit, offset),
            )  # noqa: S608
            rows = cur.fetchall()
    ...
    def update_report(self, project_id: str, report_id: str, updates: dict) -> dict | None:
        ...
        with self._transaction() as cur:
            cur.execute(
                "UPDATE reports SET " + ", ".join(set_clauses) + " WHERE id = ? AND project_id = ?",
                values,
            )  # noqa: S608
