...
function verifyPythonPackages(python: string): boolean {
                ...
                if (result.status !== 0) {
                        ...
                        if (app.isReady()) {
                                dialog.showErrorBox(
                                        ...
                                        `The Python backend requires packages that are not installed.\n\n` +
                                        `Missing: ${missingPkg}\n\n` +
                                        `To fix this, run:\n` +
...
function getBackendDir(): string {
        ...
        const asarUnpacked = path.resolve(
                RESOURCES,
                "app.asar.unpacked",
                "electron",
                "backend",
...
async function waitForBackend(
        ...
        console.error(
                "[Python Backend] Health check failed after",
                retries,
                "retries",
        );
        return false;
        ...
        try {
                ...
                try {
                        ...
                        const response = await fetch(
                                "https://api.github.com/repos/ahmdelbaz28-ux/BAZspark/releases/latest",
                                { headers: { "User-Agent": "BAZspark-Updater" } },
                        );
...
app.whenReady().then(async () => {  // NOSONAR — S7785: acceptable
        ...
        if (backendReady) {
                ...
                console.warn(
                        "[App] Python backend did not start — frontend will run without API",
                );
        }
