...
export async function makeApiRequest(
  ...
  try {
    ...
    if (requestInit.body && typeof requestInit.body === 'object' && !(requestInit.body instanceof FormData)) {
...
export async function testConcurrentEndpoints(
  endpoints: string[],
  method: string = 'GET'
): Promise<Array<{ endpoint: string; response: ApiResponse; success: boolean }>> {
  const promises = endpoints.map(endpoint => 
    makeApiRequest(endpoint, { method })
      .then(response => ({
        endpoint,
        response,
