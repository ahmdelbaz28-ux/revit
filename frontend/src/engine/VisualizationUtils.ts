...
export function generateCableGradient(
  ...
  for (let i = 0; i < segments; i++) {
    const startPercent = startVoltageDrop + ((endVoltageDrop - startVoltageDrop) * (i / segments));
    const endPercent = startVoltageDrop + ((endVoltageDrop - startVoltageDrop) * ((i + 1) / segments));
