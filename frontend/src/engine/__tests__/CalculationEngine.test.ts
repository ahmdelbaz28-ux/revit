...
describe("CalculationEngine - Cable Sizing", () => {
	...
	it("should account for ambient temperature derating", () => {
		...
		expect(hot.recommendedCrossSection).toBeGreaterThanOrEqual(
			normal.recommendedCrossSection,
		);
	...
	it("should account for installation method", () => {
		...
		const directBuried = calculateCableSizing(
			30,
			"Cu",
			"direct_buried",
			30,
		...
		expect(directBuried.installationFactor).toBeLessThanOrEqual(
			conduit.installationFactor,
		);
	});
	...
	it("should require larger cable for higher current", () => {
		...
		expect(highCurrent.recommendedCrossSection).toBeGreaterThan(
			lowCurrent.recommendedCrossSection,
		);
	});
...
describe("CalculationEngine - Load Flow", () => {
	...
	it("should maintain power factor in result", () => {
		...
		const calculatedS = Math.sqrt(
			result.power ** 2 + result.reactivePower ** 2,
		);
