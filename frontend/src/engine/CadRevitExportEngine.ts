...
export function exportToRevitJSON(project: ExportProject): void {
  ...
  const blob = new Blob([JSON.stringify(revitData, null, 2)], { type: 'application/json' });
...
export function exportToIFC(project: ExportProject, ifcVersion: 'IFC2x3' | 'IFC4' = 'IFC4'): void {
  ...
  ifc += "FILE_NAME('" + project.name.replace(/'/g, "''") + ".ifc','" + timestamp + "',('" + project.author.replace(/'/g, "''") + "'),(''),'Express Data Manager Version 1.0.0','" + schema + "','');\n";
  ...
  // Application
  const appId = nextId();
  ifc += appId + "=IFCAPPLICATION($" + ",'" + schema + "','" + schema + "','NexusCAD Pro');\n";

  // Owner History
  const ownerHistoryId = nextId();
  ifc += ownerHistoryId + '=IFCOWNERHISTORY(' + getId('person') + ',' + appId + ',$,$,$,$,' + Math.floor(Date.now() / 1000) + ');\n';

  // Project
  const projectIfcId = nextId();
  ifc += projectIfcId + "=IFCPROJECT('" + projectId + "',$,'" + project.name.replace(/'/g, "''") + "',$,$,$,$,(" + getId('unit') + ")," + ownerHistoryId + ");\n";

  // Site
  const siteIfcId = nextId();
  ifc += siteIfcId + "=IFCSITE('" + siteId + "',$,'Site',$,$,$,$,$,$,$,$,$,$,$,$);\n";

  // Building
  const buildingIfcId = nextId();
  ifc += buildingIfcId + "=IFCBUILDING('" + buildingId + "',$,'Building',$,$,$,$,$,$,$,$);\n";

  // Storey
  const storeyIfcId = nextId();
  ifc += storeyIfcId + "=IFCBUILDINGSTOREY('" + storeyId + "',$,'Level 1',$,$,$,$,$,$,$);\n";

  // Containment
  ifc += nextId() + '=IFCRELAGGREGATES(' + generateGUID() + ',$,$,$,' + projectIfcId + ',(' + siteIfcId + '));\n';
  ifc += nextId() + '=IFCRELAGGREGATES(' + generateGUID() + ',$,$,$,' + siteIfcId + ',(' + buildingIfcId + '));\n';
  ifc += nextId() + '=IFCRELAGGREGATES(' + generateGUID() + ',$,$,$,' + buildingIfcId + ',(' + storeyIfcId + '));\n';
  ...
  for (const device of project.devices) {
    ...
    ifc += devId + "=" + ifcClass + "('" + generateGUID() + "',$,'" + device.name.replace(/'/g, "''") + "',$,$," + getId('placement_' + device.id) + ",''," + getId('productDef_' + device.id) + ",'" + generateGUID() + "');\n";

    // Local placement
    const placementId = getId('placement_' + device.id);
    ifc += placementId + '=IFCLOCALPLACEMENT($,' + getId('axis2_' + device.id) + ');\n';
    ifc += getId('axis2_' + device.id) + "=IFCAXIS2PLACEMENT3D(" + getId('point_' + device.id) + ",$,$);\n";
    ifc += getId('point_' + device.id) + '=IFCCARTESIANPOINT((' + device.x.toFixed(3) + ',' + device.y.toFixed(3) + ',' + device.z.toFixed(3) + '));\n';

    // Product definition
    const prodDefId = getId('productDef_' + device.id);
    ifc += prodDefId + "=IFCPRODUCTDEFINITIONSHAPE($,$,(" + getId('shape_' + device.id) + '));\n';
    ifc += getId('shape_' + device.id) + '=IFCSHAPEREPRESENTATION(' + getId('context') + ",'Body','BoundingBox',(" + getId('bbox_' + device.id) + '));\n';
    ifc += getId('bbox_' + device.id) + '=IFCBOUNDINGBOX(' + getId('point_' + device.id) + ',0.5,0.5,0.3);\n';

    // Properties
    ifc += nextId() + '=IFCPROPERTYSET(' + generateGUID() + ',$,' + "'" + device.type.replace(/'/g, "''") + " Properties',$,(" +
      nextId() + ",'" + device.category.replace(/'/g, "''") + "')," +
      nextId() + ",'" + device.voltage.toFixed(1) + "V')," +
      nextId() + ",'" + device.load.toFixed(1) + "W'));\n";
  }

  // Place devices in storey
  if (deviceIds.length > 0) {
    ifc += nextId() + '=IFCRELCONTAINEDINSPATIALSTRUCTURE(' + generateGUID() + ',$,$,$,(' + deviceIds.join(',') + '),' + storeyIfcId + ');\n';
  }

  // Context
  const contextId = getId('context');
  ifc += contextId + "=IFCGEOMETRICREPRESENTATIONCONTEXT($,'Model',3,1.E-05," + getId('direction') + ",$);\n";
  ifc += getId('direction') + '=IFCDIRECTION((1.,0.,0.));\n';

  // Units
  const unitId = getId('unit');
  ifc += unitId + '=IFCUNITASSIGNMENT((' + getId('lengthUnit') + ',' + getId('areaUnit') + ',' + getId('volumeUnit') + '));\n';
  ...
  ifc += personId + "=IFCPERSON($,'" + project.author.replace(/'/g, "''") + "',$,$,$,$,$,$);\n";
