...
import { CanvasElement, Device, Connection } from '@/store/simpleStore';
...
const SAFETY_MARGIN = 1.10; // 10% extra for bends and termination
const CABLE_WEIGHT_PER_MM2_KG = 0.009; // kg per meter per mm² (Cu)
const CABLE_COST_PER_METER = 0.15; // EUR per meter base cost

const CONDUIT_SIZES = ['16', '20', '25', '32', '40', '50', '63', '75', '90', '110'];
const CONDUIT_AREAS: Record<string, number> = {
  '16': 78, '20': 133, '25': 230, '32': 387, '40': 582, 
  '50': 862, '63': 1376, '75': 1970, '90': 2880, '110': 4210
};
const MAX_FILL_RATIO = 0.40; // 40% fill per IEC/NEC
...
export function generateCableSchedule(
  ...
  connections.forEach((conn, index) => {
    ...
    if (fromDevice.type.includes('SENSOR') || toDevice.type.includes('SENSOR')) {
      cableType = 'signal';
    }
...
export function calculateConduitSize(
  ...
  cables.forEach((cable, i) => {
    const cableArea = (Math.PI / 4) * Math.pow(Math.sqrt(cable.crossSection / Math.PI) * 2 * 1.13 * 10, 2);
...
export function generateBomSummary(
  ...
  const totalWeight = cableSchedule.reduce((sum, c) => sum + c.estimatedWeight, 0);
  ...
  return {
    ...
    estimatedCost: Math.round((totalCost + conduitCost + deviceCost) * 100) / 100,
...
export function generateDxfDeviceEntities(
  devices: Device[]
): DxfEntity[] {
