...
// F-12 FIX (Engineering Review): SMOKE_DETECTOR_MAX_SPACING was 7.5m, which is
// WRONG per NFPA 72-2022 §17.7.3.2.3 — the canonical value is 9.1m (30ft) on
// smooth ceilings. The 7.5m value caused the frontend to over-densify smoke
// detector placements by ~21% versus the backend (which correctly uses 9.1m
// from fireai.constants.nfpa72.SMOKE_MAX_SPACING_M). This divergence meant the
// frontend would flag a compliant backend design as non-compliant, and an
// engineer modifying the design in the UI could end up with MORE detectors
// than NFPA 72 requires — confusing and economically wasteful.
// The wall-distance constant is updated to half the spacing (0.5 × 9.1 = 4.55m)
// per NFPA 72 §17.7.3.2.3 ("distance from wall to nearest detector shall not
// exceed one-half the nominal spacing").
const SMOKE_DETECTOR_MAX_SPACING = 9.1; // meters (NFPA 72-2022 §17.7.3.2.3, 30ft)
const _SMOKE_DETECTOR_MAX_FROM_WALL = 4.55; // meters (0.5 × 9.1m per §17.7.3.2.3)
const _HEAT_DETECTOR_MAX_SPACING = 6.1; // meters (NFPA 72 §17.6.3.1, 20ft standard)
const DETECTOR_COVERAGE_AREA = 81; // m² per detector (π × R² = π × (0.7×9.1)² = 127.5m² spacing cell ≈ 9.1×9.1=82.8m²)
...
/**
 * Validate smoke detector placement per NFPA 72-2022 Chapter 17
 * Rules:
 * - Max 9.1m spacing between detectors (§17.7.3.2.3, 30ft)
 * - Max 4.55m from any wall (0.5 × 9.1m, §17.7.3.2.3)
 * - Coverage area max ~81m² per detector (9.1 × 9.1m spacing cell)
 */
        ...
        return {
                ...
                status:
                        violations.filter((v) => v.severity === "CRITICAL").length > 0  // NOSONAR: typescript:S7754
                                ? "FAIL"
                                : violations.length > 0  // NOSONAR: typescript:S3358
                                        ? "FAIL"
                                        : "PROPER",
                violations,
        };
...
// F-11 FIX (Engineering Review): the previous `validateAllDevices(devices)`
// signature did not accept any room/context information, so it hardcoded
// room dimensions (15×20m) and ceiling height (2.5m) for every validation.
// This produced false violations on larger rooms (15×20 was too small) and
// missed real violations on smaller rooms. The function now accepts an
// optional `roomContext` parameter; if omitted, it falls back to a clearly
// marked "default audit" mode that emits a warning violation so the caller
// knows the dimensions are not real.
export interface RoomContextForValidation {
        width: number;       // meters
        length: number;      // meters
        ceilingHeight: number; // meters
}

export function validateAllDevices(
        devices: Device[],
        roomContext?: RoomContextForValidation,
): CodeViolation[] {
        const violations: CodeViolation[] = [];

        // F-11 FIX: if no room context was supplied, emit a WARNING violation
        // instead of silently using hardcoded dimensions. This makes the audit
        // output honest — the caller knows the spacing check was done against
        // a generic 15×20m room, not the real room geometry.
        const room = roomContext ?? { width: 15, length: 20, ceilingHeight: 2.5 };
        if (!roomContext) {
                violations.push({
                        id: "AUDIT_CONTEXT_MISSING",
                        standard: "NFPA72" as Standard,
                        rule: "AUDIT_CONTEXT_MISSING",
                        section: "§17.7.3.2.3",
                        severity: "WARNING" as Severity,
                        message:
                                "validateAllDevices was called without room dimensions — " +
                                "spacing checks are using a default 15×20m × 2.5m room. " +
                                "Pass real roomContext { width, length, ceilingHeight } for " +
                                "an accurate NFPA 72 §17.7.3.2.3 spacing audit.",
                        affectedDevices: [],
                        recommendation:
                                "Pass real room dimensions to validateAllDevices so the " +
                                "spacing check uses actual room geometry.",
                        autoFixable: false,
                });
        }
        ...
        // Validate smoke detector placement — use actual room dimensions
        ...
        // Validate emergency lighting — use actual room length + ceiling height
        if (emergencyLights.length > 0) {
                const lightResults = validateEmergencyLighting(
                        emergencyLights,
                        room.length,
                        room.ceilingHeight,
                );
