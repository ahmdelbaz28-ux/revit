...
import { Device } from '@/store/simpleStore';
  ...
  if (detectors.length === 0) {
    return [{
      ...
      violations: [{
        ...
        affectedDevices: [],
        recommendation: 'Install smoke detectors following NFPA 72 spacing requirements',
        autoFixable: true
      }]
    }];
  }
  ...
  for (let i = 0; i < detectors.length; i++) {
    for (let j = i + 1; j < detectors.length; j++) {
      const dist = calculateDistance(
        detectors[i].x, detectors[i].y,
        detectors[j].x, detectors[j].y
      );
      ...
      results.push({
        ...
        violations: isCompliant ? [] : [{
          ...
          recommendation: 'Add additional smoke detector or relocate existing ones',
          autoFixable: false
        }]
      });
    }
    ...
    if (distFromWall < DEAD_AIR_SPACE_MIN) {
      results.push({
        ...
        violations: [{
          ...
          recommendation: 'Move detector away from wall — at least 0.1m clearance required',
          autoFixable: false
        }]
      });
    }
...
export function validateEmergencyLighting(
  ...
  if (mountingHeight < EMERGENCY_LIGHT_MIN_HEIGHT || mountingHeight > EMERGENCY_LIGHT_MAX_HEIGHT) {
    results.push({
      isCompliant: false,
    ...
    for (let j = i + 1; j < lights.length; j++) {
      ...
      results.push({
        ...
        violations: isCompliant ? [] : [{
          ...
          recommendation: 'Add intermediate emergency light or reposition existing lights',
          autoFixable: false
        }]
      });
    }
  ...
  const effectiveSpacing = mountingHeight > 3 ? EMERGENCY_LIGHT_SPACING * 0.9 : EMERGENCY_LIGHT_SPACING;
...
export function validateCableProtection(
  ...
  return {
    ...
    status: violations.filter(v => v.severity === 'CRITICAL').length > 0 ? 'FAIL' : 
            violations.length > 0 ? 'FAIL' : 'PROPER',
    violations
  };
}
...
export function validateMotorCircuit(
  ...
  const ampacity: Record<number, number> = {
    1.5: 18, 2.5: 25, 4: 32, 6: 41, 10: 54, 16: 73,
    25: 95, 35: 115, 50: 140, 70: 175, 95: 210, 120: 245
  };
  ...
  if (!breakerOk) {
    violations.push({
      ...
      affectedDevices: [],
      recommendation: 'Select breaker from standard sizes to properly protect motor circuit',
      autoFixable: true
    });
  }
  ...
  return {
    ...
    serviceFactor: 1.15,
    minWireSize: Math.ceil(minWireAmpacity / 10) * 10 > 120 ? 120 : Math.ceil(minWireAmpacity / 10) * 10,
...
export function validatePanelSizing(
  ...
  if (usagePercent > 100) {
    violations.push({
      ...
      affectedDevices: [],
      recommendation: 'URGENT: Add panel capacity or reduce connected load immediately',
      autoFixable: false
    });
  }
...
export function validateAllDevices(devices: Device[]): CodeViolation[] {
  ...
  if (smokeDetectors.length > 1) {
    const spacingResults = validateSmokeDetectorPlacement(smokeDetectors, 15, 20);
