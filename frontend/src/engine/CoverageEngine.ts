...
export function validateDetectorPlacement(
        ...
        detectors.forEach((detector) => {
                ...
                // Check wall distance
                // F-07 FIX (Engineering Review): the canonical NFPA 72 minimum
                // wall clearance for detector placement is 0.1016m (4 inches)
                // per fireai.constants.nfpa72.WALL_MIN_DISTANCE_M. The previous
                // 0.5m threshold was both too strict (flagged valid placements
                // as "close to wall") AND not the canonical value. We use the
                // canonical 4-inch value here; placements closer than 4 inches
                // to a wall are physically non-compliant (dead-air space).
                const WALL_MIN_DISTANCE_M = 0.1016; // 4 inches per NFPA 72 §17.7.3.2.3
                if (
                        detector.x < WALL_MIN_DISTANCE_M ||
                        detector.y < WALL_MIN_DISTANCE_M ||
                        detector.x > room.width - WALL_MIN_DISTANCE_M ||
                        detector.y > room.length - WALL_MIN_DISTANCE_M
                ) {
                        warnings.push(
                                `Detector ${detector.id} is closer than ${WALL_MIN_DISTANCE_M}m (4in) to a wall — NFPA 72 §17.7.3.2.3 requires at least 4 inches clearance from walls (dead-air space rule).`,
                        );
                }
