...
const DEFAULT_LAYERS: DrawingLayer[] = [
  { id: 'layer_devices', name: 'Devices', visible: true, locked: false, color: '#3b82f6', opacity: 1, order: 0 },
  { id: 'layer_wires', name: 'Wiring', visible: true, locked: false, color: '#f59e0b', opacity: 1, order: 1 },
  { id: 'layer_conduits', name: 'Conduits', visible: true, locked: false, color: '#10b981', opacity: 0.8, order: 2 },
  { id: 'layer_cable_trays', name: 'Cable Trays', visible: true, locked: false, color: '#8b5cf6', opacity: 0.8, order: 3 },
  { id: 'layer_dimensions', name: 'Dimensions', visible: true, locked: false, color: '#94a3b8', opacity: 1, order: 4 },
  { id: 'layer_annotations', name: 'Annotations', visible: true, locked: false, color: '#64748b', opacity: 1, order: 5 },
  { id: 'layer_architecture', name: 'Architecture', visible: true, locked: false, color: '#475569', opacity: 0.5, order: 6 },
];
...
export function useDrawing() {
  ...
  const setGridSize = useCallback((size: number) => {
    setState((prev) => ({ ...prev, gridSize: Math.max(5, Math.min(100, size)) }));
  }, []);
  ...
  const zoomIn = useCallback(() => {
    setState((prev) => ({
      ...prev,
      viewport: { ...prev.viewport, zoom: Math.min(10, prev.viewport.zoom * 1.2) },
    }));
  }, []);
  ...
  const zoomOut = useCallback(() => {
    setState((prev) => ({
      ...prev,
      viewport: { ...prev.viewport, zoom: Math.max(0.1, prev.viewport.zoom / 1.2) },
    }));
  }, []);
  ...
  const handleCanvasClick = useCallback(
    (screenX: number, screenY: number) => {
      ...
      const snappedPoint = snapPoint(worldPoint, state.gridSize, state.snapToGrid);
      ...
      if (state.activeTool === 'erase') {
        const hitElement = findElementAtPoint(worldPoint, state.elements, state.layers);
        if (hitElement) {
          pushHistory();
          setState((prev) => ({
            ...prev,
  ...
  const addDevice = useCallback(
    (deviceId: string, x: number, y: number) => {
      pushHistory();
      const snappedPoint = snapPoint({ x, y }, state.gridSize, state.snapToGrid);
      const newElement: DrawingElement = {
        id: generateId(),
        type: 'device',
        layerId: state.activeLayerId,
  ...
  const importDrawing = useCallback(
    (data: { elements: DrawingElement[]; layers: DrawingLayer[]; viewport: Viewport; gridSize: number }) => {
      pushHistory();
      setState((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        if (state.selectedIds.length > 0 && document.activeElement === document.body) {
          e.preventDefault();
          deleteSelected();
        }
      }
