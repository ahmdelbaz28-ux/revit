...
export function useReportManager() {
        ...
        const generateReport = useCallback(
                (
                        ...
                        // V211: SonarCloud S2245 — use Web Crypto API for report ID generation
                        const _randBytes = new Uint8Array(4);
                        crypto.getRandomValues(_randBytes);
                        const _rand = Array.from(_randBytes, (b) => b.toString(36).padStart(2, "0")).join("").slice(0, 6);
                        return {
                                id: `RPT_${Date.now()}_${_rand}`,
                                type,
                                name: template.name,
                                timestamp: new Date(),
