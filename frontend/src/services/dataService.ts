...
export class DataService {
        ...
        // API key for WebSocket authentication (in-memory only — V243 SECURITY)
        // V243 SECURITY FIX: API keys are NO LONGER stored in localStorage.
        // localStorage is readable by any XSS payload — storing a long-lived API
        // key there means a single XSS gives full account takeover. The canonical
        // auth flow uses an HttpOnly session cookie (set by POST /auth/login),
        // which JavaScript cannot read. This in-memory field is only used as a
        // transient pass-through for WebSocket connections that require an API
        // key header (the cookie can't be attached to WS handshakes in some
        // browsers). The key is cleared on page reload.
        private apiKey: string | null = null;

        private constructor() {
                // V243: Do NOT load from localStorage. The key is set transiently
                // via setApiKey() after login and cleared on reload.
        }
        ...
        /** Set the API key for WebSocket authentication (in-memory only) */
        public setApiKey(key: string | null) {
                this.apiKey = key;
                // V243 SECURITY: Do NOT persist to localStorage. The key lives only
                // in memory for the current session and is cleared on page reload.
        }
