...
export class DataService {
  ...
  public connect(mode?: 'live' | 'simulation' | 'demo' | 'mock') {
    ...
    actions.addLog(`Attempting to connect to Live Data Server (${this.dataSource} mode)...`);
    ...
    try {
      ...
      this.ws.onclose = (event) => {
        ...
        if (wasConnected || wasAuth) {
          actions.addLog(`Connection lost (${reason}). Buffering incoming data...`);
        } else {
        ...
        if (event.code === 4003) {
          actions.addLog("[WARNING] Authentication failed. Check your API key or switch to demo mode.");
          return;
        }

        // Don't reconnect if origin rejected
        if (event.code === 4001) {
          actions.addLog("[WARNING] Origin rejected by server. Check CORS configuration.");
          return;
        }
        ...
        if (!event.wasClean && this.reconnectAttempts < WS_MAX_RECONNECT_ATTEMPTS) {
          ...
          const delay = Math.min(
            WS_RECONNECT_INTERVAL * Math.pow(1.5, this.reconnectAttempts - 1),
            60000
          );
          actions.addLog(
          ...
          actions.addLog("[WARNING] Max reconnection attempts reached. Falling back to mock data.");
          this.fallbackToMock();
        }
      };
    } catch (error) {
      actions.addLog("[ERROR] Failed to create WebSocket connection. Falling back to mock data.");
      this.fallbackToMock();
    }
  }
  ...
  private handleWebSocketMessage(message: any) {
    ...
    if (message.channel === "system" && message.type === "auth_failed") {
      actions.addLog("[ERROR] WebSocket authentication failed. Check your API key.");
      this.isAuthenticated = false;
      if (this.ws) {
    ...
    if (message.channel === "sync" && message.type === "sync_completed") {
      if (import.meta.env.DEV) {
        console.log("[DataService] Sync completed for project:", message.projectId);
      }
    ...
    if (import.meta.env.DEV) {
      console.log("[DataService] Unhandled message:", message.channel, message.type);
    }
  }
  ...
  private connectMockWorker() {
    this.isConnected = true;
    actions.setConnectionStatus("connected");
    actions.addLog("Connected to Live Data Server (DEMO MODE — simulated data).");
    ...
    this.mockWorker = new Worker(new URL("./mockWorker.ts", import.meta.url), { type: "module" });
  ...
  public isConnectedLive(): boolean {
    return this.isConnected && this.isAuthenticated && this.dataSource === 'live' && this.ws?.readyState === WebSocket.OPEN;
  }
  ...
  public sendCommand(action: string, payload?: any) {
    if (this.ws?.readyState === WebSocket.OPEN && this.isAuthenticated) {
      this.ws.send(JSON.stringify({ action, ...payload, timestamp: Date.now() }));
    } else {
      actions.addLog("[WARNING] Cannot send command — not authenticated or not connected.");
    }
  }
}
