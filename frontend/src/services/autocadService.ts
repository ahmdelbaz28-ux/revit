...
export const autocadService = {
        ...
        /** POST /autocad/draw_polyline — Draw a polyline (V221: points→vertices) */
        drawPolyline: (vertices: number[][], layer?: string) =>
                autocadApi.drawPolyline({ vertices, layer }),
        ...
        /** POST /autocad/draw_text — Draw text (V221: point→insertion_point) */
        drawText: (insertionPoint: number[], text: string, height?: number, layer?: string) =>
                autocadApi.drawText({ text, insertion_point: insertionPoint, height, layer }),

        /** GET /autocad/status — Get connection status */
        getStatus: () => autocadApi.getStatus(),

        /** POST /autocad/save — Save document (V221: filepath required) */
