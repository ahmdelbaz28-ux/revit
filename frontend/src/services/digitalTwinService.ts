...
export const digitalTwinService = {
  ...
  convert: (sourceFile: string, targetFormat: string) =>
    digitalTwinApi.convert({ source_file: sourceFile, target_format: targetFormat }),
  ...
  configure: (config: Record<string, unknown>) => digitalTwinApi.configure(config),
  ...
  updateMapping: (mapping: Record<string, unknown>) => digitalTwinApi.updateMapping(mapping),

  /** GET /digital-twin/config — Get current config */
  getConfig: () => digitalTwinApi.getConfig(),

  /** PUT /digital-twin/config — Set config */
  setConfig: (config: Record<string, unknown>) => digitalTwinApi.setConfig(config),
