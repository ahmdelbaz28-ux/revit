...
import { useStore, actions, Device, Connection } from '@/store/simpleStore';
import { AlertTriangle, Zap, CheckCircle, Activity, Wrench } from 'lucide-react';
...
export function SystemAnalyzer() {
  ...
  const analyzeSystem = () => {
    ...
    connections.forEach(conn => {
      if (conn.isOverloaded) {
        foundIssues.push({
          ...
          solution: 'Upgrade cable cross-section or redistribute load to adjacent panels.',
    ...
    devices.forEach(dev => {
      const isConnected = connections.some(c => c.fromId === dev.id || c.toId === dev.id);
      if (!isConnected && dev.type !== 'GENERATOR') { // Generators can be standalone initially
        foundIssues.push({
          id: `ISSUE-ISO-${dev.id}`,
          type: 'warning',
          title: 'Isolated Component',
          description: `${dev.type} ${dev.id} has no electrical connections.`,
          solution: 'Connect component to the nearest distribution panel or source.',
          relatedIds: [dev.id]
        });
      }
    });
  ...
  const applyAutoFix = (issue: Issue) => {
    if (issue.title.includes('Overload')) {
      ...
      alert(`Focused on ${issue.relatedIds[0]}. Please drag a connection from a nearby panel.`);
    }
  };
  ...
  return (
    <div className="p-4 space-y-4 h-full overflow-y-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button onClick={analyzeSystem} size="sm" className="bg-blue-600 hover:bg-blue-700">
          Run Diagnostics
        </Button>
      </div>
        ...
        <div className="space-y-3">
          {issues.map(issue => (
            <div key={issue.id} className={`p-3 rounded-lg border ${
              issue.type === 'critical' ? 'bg-red-900/20 border-red-500/50' : 'bg-amber-900/20 border-amber-500/50'
            }`}>
              <div className="flex items-start gap-3">
                <AlertTriangle className={`h-5 w-5 shrink-0 ${issue.type === 'critical' ? 'text-red-500' : 'text-amber-500'}`} />
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-background/50 p-2 rounded text-xs border border-border">
                    <span className="font-bold text-blue-400">💡 Suggestion:</span> {issue.solution}
                  </div>
                  <Button 
