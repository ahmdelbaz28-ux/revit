...
import { Shield, Activity, AlertTriangle, Layers, TrendingUp, ShieldAlert, Settings } from 'lucide-react';
...
export function NexusWorkspace() {
  ...
  return (
    <div className="flex h-screen w-screen bg-background text-foreground overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative border-r border-border" onDragEnd={() => setDraggedItem(null)}>
        {/* Top Header */}
        <div className="h-14 border-b border-border flex items-center justify-between px-6 bg-card/50 backdrop-blur-md shrink-0">
          ...
          <div className="flex items-center gap-4 text-xs">
             <span className="text-muted-foreground flex items-center gap-1"><Layers size={12}/> Auto-Save: Active</span>
             {hasCriticalErrors && (
        ...
        {errors.length > 0 && (
          <div className="absolute bottom-4 left-4 right-4 bg-destructive/10 border border-destructive/50 backdrop-blur-md rounded-lg p-3 max-h-48 overflow-y-auto shadow-2xl z-20">
            <div className="flex justify-between items-center mb-2 sticky top-0 bg-transparent backdrop-blur-sm z-10">
              ...
              <button onClick={() => actions.clearErrors()} className="text-[10px] text-destructive hover:underline hover:text-destructive-foreground transition-colors">Clear All</button>
            </div>
            {errors.map(err => (
              <div key={err.id} className="text-xs text-destructive mb-1.5 flex items-start gap-2 font-mono">
                <span className="opacity-50 shrink-0">[{new Date(err.timestamp).toLocaleTimeString()}]</span>
