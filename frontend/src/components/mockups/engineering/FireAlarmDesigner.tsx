...
import { CanvasEditor } from '@/components/firealarm/CanvasEditor';
import { Detector } from '@/components/firealarm/CanvasEditor';
...
export function FireAlarmDesigner() {
  ...
  useEffect(() => {
    setDetectors([
      { id: 'det-1', x: 150, y: 200, type: 'smoke', status: 'normal', coverageRadius: 6.37 },
      { id: 'det-2', x: 300, y: 150, type: 'heat', status: 'warning', coverageRadius: 4.27 },
      { id: 'det-3', x: 450, y: 250, type: 'pull', status: 'normal', coverageRadius: 0 },
    ]);
  }, []);

  const handleAddDetector = (type: 'smoke' | 'heat' | 'pull' | 'horns' | 'speaker' | 'facp') => {
  ...
  const handleDetectorUpdate = (updatedDetector: Detector) => {
    setDetectors(detectors.map(det => det.id === updatedDetector.id ? updatedDetector : det));
    ...
    <div className="flex-1 overflow-auto" aria-label={t('fireAlarm.designer')}>
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-100">{t('fireAlarm.designer')}</h1>
            <p className="text-sm text-slate-400 mt-1">{t('fireAlarm.designerSubtitle')}</p>
          </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-slate-300">{t('fireAlarm.projectName')}</Label>
                <Input
                  value={projectName}
              ...
              <div className="space-y-2">
                <Label className="text-slate-300">{t('fireAlarm.description')}</Label>
                <Input
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  className={showGrid ? "bg-slate-600 hover:bg-slate-700" : "border-slate-600 text-slate-300 hover:bg-slate-800"}
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  className={snapToGrid ? "bg-slate-600 hover:bg-slate-700" : "border-slate-600 text-slate-300 hover:bg-slate-800"}
                  onClick={() => setSnapToGrid(!snapToGrid)}
                >
                  {snapToGrid ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}
                </Button>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Select value={units} onValueChange={(v: 'metric' | 'imperial') => setUnits(v)}>
                  ...
                  <SelectContent className="bg-slate-800 border-slate-700">
                    <SelectItem value="metric">{t('fireAlarm.metric')}</SelectItem>
                    <SelectItem value="imperial">{t('fireAlarm.imperial')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-slate-900/50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-slate-100">{detectors.length}</div>
                <div className="text-sm text-slate-400">{t('fireAlarm.totalDetectors')}</div>
              </div>
              <div className="bg-slate-900/50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-emerald-400">
                  {detectors.filter(d => d.status === 'normal').length}
                </div>
                <div className="text-sm text-slate-400">{t('fireAlarm.normal')}</div>
              </div>
              <div className="bg-slate-900/50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-amber-400">
                  {detectors.filter(d => d.status === 'warning').length}
                </div>
                <div className="text-sm text-slate-400">{t('fireAlarm.warning')}</div>
              </div>
              <div className="bg-slate-900/50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-red-400">
                  {detectors.filter(d => d.status === 'fault').length}
                </div>
                <div className="text-sm text-slate-400">{t('fireAlarm.fault')}</div>
              </div>
            </div>
          </CardContent>
        </Card>
