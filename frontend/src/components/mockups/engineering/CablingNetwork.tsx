			...
			<div className="h-24 flex flex-col border-b border-slate-700 bg-slate-800 shrink-0">
				<div className="h-10 flex items-center justify-between px-4 border-b border-slate-700/50 bg-slate-900/50">
					<div className="flex items-center gap-4">
						...
						<div className="flex text-xs space-x-1">
							<button className="px-3 py-1 rounded-sm transition-colors bg-emerald-500/10 text-emerald-400 font-medium border border-emerald-500/20">
								Structured Cabling
							</button>
							<button className="px-3 py-1 rounded-sm transition-colors text-slate-400 hover:bg-slate-800 hover:text-slate-200">
								Network Design
							</button>
							<button className="px-3 py-1 rounded-sm transition-colors text-slate-400 hover:bg-slate-800 hover:text-slate-200">
								Wi-Fi
				...
				<div className="flex-1 flex items-center px-2 space-x-1 bg-slate-800 overflow-x-auto">
					...
					<div className="flex items-center px-2 border-r border-slate-700">
						<RibbonBtn
							icon={<LayoutGrid className="text-slate-300" />}
							label="Data Outlet"
							active
						/>
						<RibbonBtn
							icon={<Monitor className="text-slate-300" />}
							label="Voice Outlet"
						/>
						<RibbonBtn
							icon={<MonitorPlay className="text-slate-300" />}
							label="AV Outlet"
						/>
					</div>
					{/* Active Equip */}
					<div className="flex items-center px-2 border-r border-slate-700">
						<RibbonBtn
							icon={<ArrowRightLeft className="text-slate-300" />}
							label="Switch"
						/>
						<RibbonBtn
							icon={<Layout className="text-slate-300" />}
							label="Patch Panel"
						/>
						<RibbonBtn
							icon={<Wifi className="text-slate-300" />}
							label="Wi-Fi AP"
						/>
					</div>
					{/* Pathways & Rooms */}
					<div className="flex items-center px-2 border-r border-slate-700">
						<RibbonBtn
							icon={<Cable className="text-emerald-400" />}
							label="Cable Tray"
						/>
						<RibbonBtn
							icon={<Server className="text-slate-300" />}
							label="Telecom Room"
						/>
					</div>
					{/* AV */}
					<div className="flex items-center px-2 border-r border-slate-700">
						<RibbonBtn
							icon={<Speaker className="text-slate-300" />}
							label="Speaker"
						/>
						<RibbonBtn
							icon={<Mic className="text-slate-300" />}
							label="Microphone"
						/>
						<RibbonBtn
							icon={<Volume2 className="text-slate-300" />}
							label="Amplifier"
						/>
					</div>
				</div>
			...
			<div className="flex flex-1 overflow-hidden">
				...
				<div className="w-[240px] flex flex-col border-r border-slate-700 bg-slate-800 shrink-0">
					...
					<div className="p-3 border-t border-slate-700 bg-slate-900/50 text-xs">
						<div className="text-slate-400 mb-1 uppercase tracking-wider font-semibold">
							Summary
						</div>
						<div className="grid grid-cols-2 gap-1 text-slate-300 font-mono">
							<div>
								Data: <span className="text-slate-100">476</span>
							</div>
							<div>
								Voice: <span className="text-slate-100">124</span>
							</div>
							<div>
								AV: <span className="text-slate-100">68</span>
							</div>
							<div>
								Wi-Fi: <span className="text-slate-100">48</span>
							</div>
						</div>
					</div>
				</div>
				...
				>
					<div className="absolute inset-0 flex items-center justify-center p-8">
						<div className="relative w-full max-w-3xl aspect-[4/3] border border-slate-700 bg-slate-800/20">
							{/* Floor Plan Outlines */}
							<div className="absolute top-10 left-10 w-[250px] h-[300px] border border-slate-600 bg-slate-800/30"></div>{" "}
							{/* Office A */}
							<div className="absolute top-10 right-10 w-[250px] h-[300px] border border-slate-600 bg-slate-800/30"></div>{" "}
							{/* Office B */}
							<div className="absolute bottom-10 left-10 w-[150px] h-[100px] border border-slate-600 bg-blue-500/10"></div>{" "}
							{/* Meeting 201 */}
							<div className="absolute top-10 left-[45%] w-[10%] h-[150px] border-2 border-emerald-500/50 bg-emerald-500/10 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.1)]">
								<span className="text-[8px] text-emerald-400 font-bold uppercase rotate-90 tracking-widest">
									IDF-L2
								</span>
							</div>
							...
							{/* Labels */}
							<div className="absolute left-[120px] top-[210px] text-[8px] text-emerald-300 bg-slate-900 px-1 font-mono -translate-x-1/2 border border-emerald-500/30 rounded">
								DA-2-047
							</div>
						</div>
					</div>
						...
						<div className="space-y-4">
							...
							<div>
								<div className="flex justify-between items-center mb-2">
									...
									<Badge className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-[9px] h-4 py-0 px-1 font-mono">
										PASS
									</Badge>
								</div>
								<div className="space-y-1 bg-slate-900/50 p-2 rounded border border-slate-700/50 text-xs font-mono text-[10px]">
									...
									<div className="flex justify-between text-slate-400">
										...
										<span className="text-emerald-400">2.1 dB</span>
									</div>
									<div className="flex justify-between text-slate-400">
										<span>NEXT</span>{" "}
										<span className="text-emerald-400">42.1 dB</span>
									</div>
							...
							<div>
								<h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1">
			...
			<div className="h-8 bg-slate-900 border-t border-slate-700 flex items-center justify-between px-4 text-[10px] font-mono shrink-0">
				<div className="flex items-center gap-4 text-slate-400">
					...
					<span className="flex items-center gap-1">
						<Wifi className="w-3 h-3 text-blue-400" /> 98.2% cvg @ -65dBm
					</span>
				</div>
				<div className="flex items-center gap-3">
					<Button
						variant="ghost"
						size="sm"
						className="h-6 text-[10px] text-emerald-400 hover:text-emerald-300 px-2"
					>
						Cable Schedule
					</Button>
					<Button
...
function RibbonBtn({  // NOSONAR - typescript:S6759
	...
	return (
		<div
			className={`flex flex-col items-center justify-center w-16 h-12 rounded cursor-pointer transition-colors group ${active ? "bg-emerald-500/20 border border-emerald-500/30" : "hover:bg-slate-700"}`}
		>
			<div className="mb-1 [&>svg]:w-4 [&>svg]:h-4">{icon}</div>
			<span
				className={`text-[9px] text-center leading-tight whitespace-nowrap px-1 overflow-hidden text-ellipsis w-full ${active ? "text-emerald-400" : "text-slate-400 group-hover:text-slate-200"}`}
			>
				{label}
			</span>
		</div>
...
function NavNode({  // NOSONAR - typescript:S6759
	...
	return (
		<div className="select-none">
			<div
				className={`flex items-center gap-1.5 py-1 px-2 rounded cursor-pointer hover:bg-slate-700/50 ${active ? "bg-emerald-500/20 text-emerald-400 font-medium" : "text-slate-300"}`}
			>
...
function PropRow({ label, value }: { label: string; value: string }) {  // NOSONAR - typescript:S6759
	return (
		<div className="flex justify-between py-0.5">
			...
			<span className="text-slate-200 text-right font-mono text-[10px]">
				{value}
			</span>
		</div>
	);
