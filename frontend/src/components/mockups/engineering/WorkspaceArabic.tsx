...
import { 
  FileText, CheckSquare, Zap, Activity, HardDrive, Share2, Search,
...
export function WorkspaceArabic() {
  ...
  return (
    <div dir="rtl" className="flex flex-col h-screen w-screen overflow-hidden bg-background text-foreground font-sans">
      {/* Top Ribbon */}
        ...
        <div className="h-10 flex items-center justify-between px-4 border-b border-border/50">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-primary">
              ...
              <span className="font-bold tracking-wider text-sm">نيكساد برو</span>
            </div>
            <Separator orientation="vertical" className="h-5" />
            <div className="flex text-xs space-x-1">
              {["ملف", "تحرير", "عرض", "إدراج", "تعليق", "الإنشائي", "كهربائي", "نمذجة معلومات البناء", "الذكاء الاصطناعي", "تعاون", "أدوات", "مساعدة"].map((tab) => (
                <button
                  key={tab}
          ...
          <div className="flex items-center gap-4 text-muted-foreground">
            ...
            <div className="flex bg-muted/50 rounded-full p-0.5 mr-4 border border-border/50">
              <div className="px-2.5 py-0.5 text-[10px] font-semibold text-muted-foreground rounded-full hover:bg-muted cursor-pointer transition-colors">EN</div>
              <div className="px-2.5 py-0.5 text-[10px] font-semibold text-primary-foreground bg-primary rounded-full shadow-sm">عربي ✓</div>
              <div className="px-2.5 py-0.5 text-[10px] font-semibold text-muted-foreground rounded-full hover:bg-muted cursor-pointer transition-colors">FR</div>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden" dir="ltr">
        ...
        <div className="h-14 flex items-center px-2 space-x-1 bg-card/50 overflow-x-auto">
          {activeTab === "كهربائي" && (
            <>
              ...
              <RibbonBtn icon={<Zap className="text-amber-400" />} label="وميض القوس" />
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col relative bg-[#0f1115]">
          ...
          <div className="flex border-b bg-card" dir="ltr">
            {["Tower-B-Electrical.dwg", "Fire-Alarm-Plan.dwg", "BIM-Model.rvt", "Load-Calc.xlsx"].map((file) => (
              <div 
                key={file} 
              ...
              >
                {file.endsWith('.dwg') ? <Layout className="h-3 w-3" /> : <Box className="h-3 w-3" />}
                {file}
          ...
          {/* Canvas area */}
          <div className="flex-1 relative overflow-hidden flex" style={{ backgroundImage: 'radial-gradient(#1e293b 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
            {/* Rulers */}
            <div className="absolute top-0 left-0 right-6 h-6 bg-card/80 border-b flex items-end overflow-hidden z-10" dir="ltr">
              <div className="w-full border-b border-muted-foreground/30 relative" style={{ height: '5px' }}>
                {Array.from({length: 40}).map((_, i) => (
                  <div key={i} className="absolute border-l border-muted-foreground/30 h-full" style={{ left: `${i * 50}px` }}></div>
                ))}
              </div>
            </div>
            <div className="absolute top-0 bottom-0 right-0 w-6 bg-card/80 border-l flex justify-start overflow-hidden z-10" dir="ltr">
              <div className="h-full border-r border-muted-foreground/30 relative" style={{ width: '5px' }}>
                {Array.from({length: 20}).map((_, i) => (
                  <div key={i} className="absolute border-t border-muted-foreground/30 w-full" style={{ top: `${i * 50}px` }}></div>
                ))}
              </div>
            </div>

            {/* Drawing Content */}
            <div className="absolute inset-0 pt-6 pr-6 overflow-hidden" dir="ltr">
            ...
            {/* Canvas Footer */}
            <div className="absolute bottom-4 left-4 flex items-center gap-2 z-20 font-mono text-[10px] text-muted-foreground bg-card/80 backdrop-blur px-2 py-1 rounded border" dir="ltr">
              <span>Zoom: 1:50</span>
              ...
              <div className="p-3">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground font-mono" dir="ltr">ID: ELEC-PNL-8932</div>
                  </div>
                </div>
          ...
          <div className="flex-1 flex flex-col bg-card/20">
            ...
            <ScrollArea className="flex-1 p-3">
              <div className="space-y-4">
                <div className="flex flex-col gap-1 items-start">
                  ...
                  <div className="text-[9px] text-muted-foreground" dir="ltr">14:28</div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="bg-muted px-3 py-2 rounded-lg rounded-tr-none text-xs text-foreground border">
                      ...
                      <br/><br/>
                      <span className="text-emerald-400 font-mono">الحالة: آمن</span>
                      <br/>
                      السعة المتبقية: 115A.
                    </div>
                    <div className="flex gap-1 mt-1">
                      <Badge variant="secondary" className="text-[10px] cursor-pointer hover:bg-secondary/80">عرض التفاصيل</Badge>
                      <Badge variant="secondary" className="text-[10px] cursor-pointer hover:bg-secondary/80">إنشاء تقرير</Badge>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="p-2 border-t bg-card/50">
              <div className="flex gap-1 mb-2 overflow-x-auto pb-1 scrollbar-hide">
                <Badge variant="outline" className="text-[10px] whitespace-nowrap cursor-pointer">فحص الامتثال</Badge>
                <Badge variant="outline" className="text-[10px] whitespace-nowrap cursor-pointer">اقتراح المسار</Badge>
              </div>
              <div className="relative">
                <Input className="pl-8 pr-3 bg-background border-muted text-xs h-8" placeholder="اسأل المساعد..." />
                <Button size="icon" variant="ghost" className="absolute left-0 top-0 h-8 w-8 text-primary">
      ...
      <div className={`flex flex-col border-t bg-card/95 backdrop-blur-md transition-all duration-300 ease-in-out shrink-0 overflow-hidden ${isErrorLogExpanded ? 'h-48' : 'h-7'}`}>
        ...
        <div className="h-7 flex items-center justify-between px-2 border-b cursor-pointer select-none shrink-0" onClick={() => setIsErrorLogExpanded(!isErrorLogExpanded)}>
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] font-semibold tracking-wider text-foreground">سجل الهندسة</span>
            <div className="flex gap-1.5 items-center">
              <Badge variant="destructive" className="h-4 text-[9px] px-1.5 py-0 border-red-500/50 bg-red-500">3 أخطاء</Badge>
              <Badge variant="outline" className="h-4 text-[9px] px-1.5 py-0 text-orange-400 border-orange-500/50 bg-orange-500/10">5 تحذيرات</Badge>
              <Badge variant="outline" className="h-4 text-[9px] px-1.5 py-0 text-blue-400 border-blue-500/50 bg-blue-500/10">12 معلومات</Badge>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" className="h-5 text-[10px] text-muted-foreground hover:text-foreground px-2" onClick={(e) => { e.stopPropagation(); }}>
              مسح الكل
            </Button>
            <Button variant="ghost" size="sm" className="h-5 text-[10px] text-muted-foreground hover:text-foreground px-2" onClick={(e) => { e.stopPropagation(); }}>
              تصدير السجل
            ...
            <Button variant="ghost" size="icon" className="h-5 w-5 text-muted-foreground hover:text-foreground">
              {isErrorLogExpanded ? <ChevronDown className="h-3 w-3" /> : <ChevronUp className="h-3 w-3" />}
            </Button>
          </div>
        </div>
        
        {/* Expanded Content */}
        <div className={`flex-1 overflow-hidden transition-opacity duration-200 ${isErrorLogExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <ScrollArea className="h-full w-full font-mono text-xs">
            <div className="flex flex-col">
              <LogEntry level="ERROR" time="14:47:03" source="NEC Compliance" message="لوحة LP-3A: تباعد شريط المحايد يخالف NEC §408.36 — المسافة 38مم < 50مم مطلوب" />
              <LogEntry level="ERROR" time="14:46:51" source="Load Calculator" message="MDB-B محملة زائداً: الطلب 978A يتجاوز تقييم الحافلة 800A (122.3%)" />
              <LogEntry level="ERROR" time="14:45:22" source="Arc Flash" message="لوحة LP-3A: ملصق وميض القوس مفقود. طاقة الحادثة 52.4 cal/cm² — مطلوب PPE الفئة 4" />
              <LogEntry level="WARN" time="14:44:10" source="BIM Sync" message="IFC export: 3 elements skipped — unsupported geometry type (IFCBSPLINECURVE)" />
              <LogEntry level="WARN" time="14:43:38" source="Clash Detection" message="5 new soft clashes detected in MEP zone — Level 2, Grid C–E / 3–6" />
              <LogEntry level="WARN" time="14:42:15" source="Cable Sizing" message="Cable run EL-C-047: voltage drop 4.8% approaching 5% limit" />
              <LogEntry level="WARN" time="14:41:02" source="File System" message="Auto-save delayed 12s — large file lock on BIM-Model.rvt" />
              <LogEntry level="WARN" time="14:40:30" source="Collaboration" message="User Marcus Williams connection timeout — changes may not be synced" />
              <LogEntry level="INFO" time="14:38:44" source="System" message="BIM model synchronized with Revit v14 — 2,847 elements updated" />
              <LogEntry level="INFO" time="14:37:21" source="Analysis" message="Load flow converged — Newton-Raphson, 7 iterations, tolerance 0.0001 pu" />
              <LogEntry level="INFO" time="14:36:05" source="Compliance" message="NFPA 72 check complete — 127 items passed, 2 warnings" />
              <LogEntry level="INFO" time="14:35:00" source="System" message="Auto-save complete — 5 files, 47MB" />
            </div>
          </ScrollArea>
        </div>
      </div>
...
function LogEntry({ level, time, source, message }: { level: "ERROR" | "WARN" | "INFO", time: string, source: string, message: string }) {
  const isError = level === "ERROR";
  const isWarn = level === "WARN";
  
  return (
    <div className={`flex items-center px-4 py-1.5 border-b border-border/30 hover:bg-muted/30 group ${isError ? 'bg-red-950/20 border-r-2 border-r-red-500' : isWarn ? 'bg-orange-950/20 border-r-2 border-r-orange-500' : 'border-r-2 border-r-transparent'}`}>
      <div className="w-[80px] shrink-0 text-muted-foreground" dir="ltr">{time}</div>
      <div className="w-[60px] shrink-0" dir="ltr">
        <span className={`${isError ? 'text-red-400' : isWarn ? 'text-orange-400' : 'text-slate-400'}`}>{level}</span>
      </div>
      <div className="w-[130px] shrink-0 text-slate-500 truncate pl-4" dir="ltr">{source}</div>
      ...
      <div className="shrink-0 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
        {level !== "INFO" && <span className="text-blue-400 hover:underline cursor-pointer">انتقال</span>}
...
function RibbonBtn({ icon, label }: { icon: React.ReactNode, label: string }) {
  return (
    <div className="flex flex-col items-center justify-center w-16 h-12 rounded hover:bg-muted cursor-pointer transition-colors group">
      ...
      <span className="text-[9px] text-center leading-tight whitespace-nowrap px-1 overflow-hidden text-ellipsis w-full text-muted-foreground group-hover:text-foreground">{label}</span>
    </div>
  );
}

function TreeNode({ title, children, defaultOpen = false }: { title: string, children?: React.ReactNode, defaultOpen?: boolean }) {
  ...
  return (
    <div className="select-none">
      ...
      >
        <Triangle className={`h-3 w-3 text-muted-foreground transition-transform ${open ? "rotate-180" : "-rotate-90"}`} />
...
function FileNode({ title, type, active = false }: { title: string, type: string, active?: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-2 py-1 px-2 rounded cursor-pointer ${active ? "bg-primary/10 text-primary" : "hover:bg-muted/50 text-muted-foreground"}`} dir="ltr">
      <span className={`text-xs truncate text-right w-full ${active ? "font-medium" : ""}`}>{title}</span>
      <FileText className={`h-3.5 w-3.5 shrink-0 ${color}`} />
    </div>
  );
}

function LayerRow({ name, color, active = false, hidden = false, locked = false }: { name: string, color: string, active?: boolean, hidden?: boolean, locked?: boolean }) {
  return (
    <div className={`flex items-center justify-between py-1.5 px-2 rounded hover:bg-muted/50 group ${active ? "bg-muted/30" : ""}`}>
      <div className="flex items-center gap-2">
        <div className={`w-2.5 h-2.5 rounded-full ${color}`}></div>
        <span className={`text-xs ${active ? "text-foreground font-medium" : "text-muted-foreground"}`}>{name}</span>
      </div>
      <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100">
        ...
        {locked ? <Lock className="h-3 w-3 text-amber-500/70" /> : <div className="w-3 h-3"></div>}
      </div>
    </div>
  );
}
...
function ToolBtn({ icon, active = false }: { icon: React.ReactNode, active?: boolean }) {
  return (
    <div className={`w-8 h-8 rounded flex items-center justify-center cursor-pointer [&>svg]:w-4 [&>svg]:h-4 ${active ? "bg-primary/20 text-primary border border-primary/30" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
      {icon}
    </div>
  );
}
