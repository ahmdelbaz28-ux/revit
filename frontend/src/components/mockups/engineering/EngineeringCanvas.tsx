...
import { calculateVoltageDrop, type VoltageDropResult } from '@/engine/CalculationEngine';
import { ThermalLegend, AnimatedBar } from './ThermalLegend';
import { Zap, Battery, Power, Box, Wifi, Eye, Siren } from 'lucide-react';
...
export function EngineeringCanvas({ onItemDrop }: EngineeringCanvasProps) {
  ...
  const thermalDevices = useMemo<ThermalDevice[]>(() => {
    return devices.map(dev => {
      ...
      const totalCurrent = deviceConnections.reduce((sum, c) => sum + c.current, 0);
      const loadPercent = (totalCurrent / 100) * 100; // Assume 100A base
      const stressColor = getLoadColor(loadPercent);
      const glowIntensity = loadPercent > 100 ? 1 : loadPercent > 80 ? 0.6 : 0.2;
    ...
    <div className="relative w-full h-full bg-[#0f1115] overflow-hidden">
      ...
      <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-sm rounded-lg p-3 border border-white/20 z-50 min-w-[200px]">
        <div className="text-xs font-bold text-white mb-2 flex items-center gap-2">
          ...
          <span className="text-[10px] bg-primary/20 text-primary px-2 py-0.5 rounded">LIVE</span>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 text-[10px]">
            ...
            <span className="font-mono text-white">{thermalConnections.length}</span>
          </div>
          
          <div className="flex items-center gap-2 text-[10px]">
            <span className="text-white/50 w-20">Critical:</span>
            <span className="font-mono text-red-400">{thermalConnections.filter(c => c.isCritical).length}</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        {thermalConnections.map(conn => {
          const gradientSegments = generateCableGradient(
            Math.sqrt(Math.pow(conn.toX - conn.fromX, 2) + Math.pow(conn.toY - conn.fromY, 2)),
          ...
          return (
            <g key={conn.id}>
              ...
              {Math.sqrt(Math.pow(conn.toX - conn.fromX, 2) + Math.pow(conn.toY - conn.fromY, 2)) > 200 && (
                <line
                  x1={conn.fromX} y1={conn.fromY}
                  x2={conn.toX} y2={conn.toY}
              ...
              {/* Midpoint Info Badge */}
              <g transform={`translate(${(conn.fromX + conn.toX) / 2}, ${(conn.fromY + conn.toY) / 2})`}>
                <rect
                  x="-25" y="-18"
                  width="50" height="18"
                  rx="4"
          ...
          return (
            ...
            >
              ...
              <rect
                ...
                rx="8"
                fill="#1e293b"
                stroke={isSelected ? '#3b82f6' : dev.isOverloaded ? '#ef4444' : dev.stressColor}
              ...
              {dev.isOverloaded && (
                <g>
                  <circle cx="50" cy="10" r="6" fill="#ef4444" className="animate-pulse" />
                  <text x="50" y="13" fill="white" fontSize="8" textAnchor="middle" fontWeight="bold">!</text>
                </g>
              )}
              ...
              {isHovered && (
                <g transform="translate(65, -20)">
                  <rect
                    ...
                    strokeWidth="1"
                  />
                  <text x="8" y="15" fill="white" fontSize="10" fontWeight="bold">Load: {dev.load}A</text>
                  <text x="8" y="28" fill="#94a3b8" fontSize="9">Voltage: {dev.voltage}V</text>
      ...
      <div className="absolute bottom-4 left-4 bg-card/90 backdrop-blur p-3 rounded border border-border shadow-lg pointer-events-none">
        <h3 className="text-xs font-bold text-primary mb-1">NexusCAD Thermal Engine</h3>
