...
import { THERMAL_SCALE, LOAD_SCALE, getVoltageDropColor, getLoadColor } from '@/engine/VisualizationUtils';
  ...
  if (compact) {
    return (
      <div
        ...
        `}
      >
        <div className="text-[10px] text-white/70 font-medium">{config.title}</div>
        ...
        <div className="flex justify-between text-[9px] text-white/50 font-mono w-24">
          <span>{scale.stops[0].value}{config.unit}</span>
          <span>{scale.stops[scale.stops.length - 1].value}{config.unit}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between text-[10px] text-white/50 font-mono mb-3">
        <span>{scale.stops[0].value}{config.unit}</span>
        <span>{Math.round((scale.stops[0].value + scale.stops[scale.stops.length - 1].value) / 2)}{config.unit}</span>
        <span>{scale.stops[scale.stops.length - 1].value}{config.unit}</span>
      </div>
      ...
      <div className="space-y-1.5">
        {config.ranges.map((range, index) => {
          const midValue = (range.min + Math.min(range.max, range.max === Infinity ? range.min + 10 : range.max)) / 2;
          ...
          return (
            <div key={index} className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-white/40 font-mono">
                {range.min}{config.unit} - {range.max === Infinity ? '∞' : range.max}{config.unit}
              </span>
            </div>
          );
        })}
      ...
      {showValues && (
        <div className="mt-3 pt-2 border-t border-white/10">
          <div className="flex justify-between text-[10px]">
            ...
            <span className="font-mono font-bold text-white" id="thermal-current-value">
              --{config.unit}
            </span>
          </div>
        </div>
...
export function MiniIndicator({ value, type, size = 'md' }: MiniIndicatorProps) {
  const color = type === 'voltageDrop' ? getVoltageDropColor(value) : getLoadColor(value);
...
export function AnimatedBar({ value, max, type, showLabel = true }: AnimatedBarProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      {showLabel && (
        <span className="text-[10px] text-white/50 w-16">{type === 'voltageDrop' ? 'V-Drop' : 'Load'}</span>
      )}
