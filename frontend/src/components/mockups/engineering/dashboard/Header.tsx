...
import { Sun, Moon, Shield, HelpCircle, Wifi, WifiOff } from "lucide-react";
...
export function Header({
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      ...
      setPacketLoss(Math.random() < 0.05 ? parseFloat((Math.random() * 0.4).toFixed(1)) : 0);
    }, 1000);
    ...
    <>
          ...
          >
            ...
            <span
              className={`h-2.5 w-2.5 rounded-full block flex-shrink-0 ${
                isConnected ? "status-pulse-connected" : "status-solid-disconnected"
              }`}
            />
            ...
            >
              ...
              <div className="px-3 py-2 space-y-2">
                <TooltipRow
                  ...
                  valueColor={
                    latency < 30 ? "#22c55e" : latency < 80 ? "#f59e0b" : "#ef4444"
                  }
                />
                <TooltipRow
                  label="Last Update"
      ...
      {!isConnected && (
        ...
        >
          ...
          <span className="text-[10px] font-mono opacity-70">ATTEMPTING RECONNECTION...</span>
        </div>
      )}
    </>
  );
...
function TooltipRow({
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <span className="text-[10px] font-mono font-semibold" style={{ color: valueColor }}>
        {value}
      </span>
    </div>
  );
