...
import { X, Save, AlertTriangle, CheckCircle2, XCircle } from 'lucide-react';
import { DetectorType, Detector as CanvasDetector } from './CanvasEditor';
...
export const DeviceProperties: React.FC<DevicePropertiesProps> = ({ 
  ...
  const [editedDevice, setEditedDevice] = useState<Partial<DeviceProperty>>({
    ...
    coverageArea: device?.coverageRadius ? Math.PI * device.coverageRadius * device.coverageRadius : 0,
    status: device?.status || 'normal',
    lastTestDate: device?.lastTestDate || new Date().toISOString().split('T')[0],
  });
  ...
  const handleSave = () => {
    if (device) {
      ...
      const updatedDevice = {
        ...
        y: device.y,
        coverageRadius: editedDevice.coverageArea ? Math.sqrt(editedDevice.coverageArea / Math.PI) : device.coverageRadius,
      };
      ...
      <Card className="border-slate-700 bg-slate-800">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div>
              <Label className="text-slate-300">{t('fireAlarm.location')}</Label>
              <Input
            ...
            <div>
              <Label className="text-slate-300">{t('fireAlarm.detectorType')}</Label>
              <Select 
                value={editedDevice.detectorType} 
                onValueChange={(value: DetectorType) => handleChange('detectorType', value)}
              >
            ...
            <div>
              <Label className="text-slate-300">{t('fireAlarm.heightAff')}</Label>
              <Input
                type="number"
                value={editedDevice.heightAFF || ''}
                onChange={(e) => handleChange('heightAFF', parseFloat(e.target.value) || 0)}
            ...
            <div>
              <Label className="text-slate-300">{t('fireAlarm.manufacturer')}</Label>
              <Input
                value={editedDevice.manufacturer || ''}
                onChange={(e) => handleChange('manufacturer', e.target.value)}
                placeholder={t('fireAlarm.manufacturerPlaceholder') || undefined}
                className="bg-slate-900 border-slate-600 text-slate-100"
              />
            </div>
            ...
            <div>
              <Label className="text-slate-300">{t('fireAlarm.sensitivity')}</Label>
              <Select 
                value={editedDevice.sensitivityLevel} 
                onValueChange={(value: 'high' | 'standard' | 'low') => handleChange('sensitivityLevel', value)}
              >
                <SelectTrigger className="bg-slate-900 border-slate-600 text-slate-100">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              <Label className="text-slate-300">{t('fireAlarm.coverageArea')}</Label>
              <Input
                type="number"
                value={editedDevice.coverageArea || ''}
                onChange={(e) => handleChange('coverageArea', parseFloat(e.target.value) || 0)}
                placeholder={t('fireAlarm.coverageAreaPlaceholder') || undefined}
                className="bg-slate-900 border-slate-600 text-slate-100"
              />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {editedDevice.status === 'normal' && <CheckCircle2 className="h-3 w-3 mr-1" />}
                  {editedDevice.status === 'warning' && <AlertTriangle className="h-3 w-3 mr-1" />}
                  {editedDevice.status === 'fault' && <XCircle className="h-3 w-3 mr-1" />}
                  {editedDevice.status}
                </Badge>
              </div>
            </div>
            ...
            <div>
              <Label className="text-slate-300">{t('fireAlarm.lastTest')}</Label>
              <Input
                type="date"
