...
export function ElementList({ elements, loading, onView, onDelete }: ElementListProps) {
        ...
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              ...
              filtered.map((el) => (
                <TableRow key={el.id} className="border-slate-700 hover:bg-slate-800/50">
                  <TableCell className="font-mono text-xs text-slate-400">{el.id}</TableCell>
                  ...
                  <TableCell className="text-slate-400">{el.level || '-'}</TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      {onView && (
                        <Button size="sm" variant="ghost" onClick={() => onView(el)}>
