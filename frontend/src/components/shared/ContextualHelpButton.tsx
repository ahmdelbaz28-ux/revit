...
export function ContextualHelpButton({ route, label }: ContextualHelpButtonProps) {
  ...
  const findHelpTopic = (): string | null => {
    ...
    const sortedRoutes = Object.keys(ROUTE_HELP_MAP).sort((a, b) => b.length - a.length);
  ...
  const handleClick = () => {
    if (!topic) {
      toast.info(i18n.language === 'ar' ? 'لا يوجد مساعدة متاحة لهذه الصفحة' : 'No help available for this page');
      return;
    }
    // Show help content in a toast (lightweight, no heavy drawer needed)
    const title = i18n.language === 'ar' ? topic.titleAr : topic.titleEn;
    const desc = i18n.language === 'ar' ? topic.descriptionAr : topic.descriptionEn;
    ...
    const helpEvent = new CustomEvent('fireai:open-help', { detail: { topicId } });
  ...
  return (
    <Button
      ...
      title={label || (i18n.language === 'ar' ? 'مساعدة هذه الصفحة' : 'Help for this page')}
    >
