...
import { History, RotateCcw, Clock, FileUp, FileDown } from 'lucide-react';
import { digitalTwinService, type VersionInfo } from '@/services/digitalTwinService';
    ...
    <Card className="border-slate-700 bg-slate-800">
      ...
      <CardContent>
        {loading ? (
          ...
          <p className="text-center text-slate-500 py-8">No conversion history available</p>
        ) : (
          <div className="space-y-3">
            {versions.map((v, idx) => (
              <div
              ...
              >
                <div className={`h-3 w-3 rounded-full mt-1 shrink-0 ${statusColors[v.status] || 'bg-slate-500'}`} />
                ...
                >
                  ...
                  {rollingBack === v.version_id ? 'Rolling back...' : 'Rollback'}
                </Button>
              </div>
            ))}
          </div>
