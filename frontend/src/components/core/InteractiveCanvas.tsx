...
import { useStore, actions, CanvasElement } from '@/store/simpleStore';
...
export function InteractiveCanvas() {
  ...
  const handleDeviceMouseUp = (e: React.MouseEvent, id: string) => {
    ...
    if (drawingFrom && drawingFrom !== id) {
      ...
      const exists = canvasElements.some(el => 
        el.type === 'cable' && 
        ((el.from === drawingFrom && el.to === id) || (el.from === id && el.to === drawingFrom))
      );
      ...
      if (!exists) {
        const newCable: CanvasElement = {
          ...
          from: drawingFrom,
          to: id,
          x: 0, y: 0, // Not used for lines directly but required by interface
        };
  ...
  const getElementById = (id: string) => canvasElements.find(el => el.id === id);
  ...
  return (
    <div className="w-full h-full bg-[#0f1115] border rounded-lg overflow-hidden relative">
      ...
      >
        {/* Render Cables */}
        {canvasElements.filter(el => el.type === 'cable').map(cable => {
        ...
        {/* Render Devices */}
        {canvasElements.filter(el => el.type !== 'cable').map(dev => {
