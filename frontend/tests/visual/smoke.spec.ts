...
const CORE_PAGES = [
  { path: '/dashboard', name: 'Dashboard', requiredText: /dashboard|compliance|status/i },
  { path: '/projects', name: 'Projects', requiredText: /project/i },
  { path: '/engineering', name: 'Engineering', requiredText: /engineering|fire|alarm/i },
  { path: '/fire-alarm', name: 'Fire Alarm', requiredText: /fire|alarm|detector/i },
  { path: '/digital-twin', name: 'Digital Twin', requiredText: /digital|twin/i },
...
for (const page of CORE_PAGES) {
  test(`${page.name} page loads without errors`, async ({ page: browserPage }) => {
    ...
    expect(errors, `Console errors on ${page.path}:\n${errors.join('\n')}`).toEqual([]);

    await captureForReview(browserPage, `01-${page.name.toLowerCase().replace(/\s/g, '-')}-loaded`);
  });
}
...
// ═══════════════════════════════════════════════════════════════════
for (const page of CORE_PAGES) {
  test(`${page.name} page has visible content`, async ({ page: browserPage }) => {
    ...
    await captureForReview(browserPage, `02-${page.name.toLowerCase().replace(/\s/g, '-')}-content`);
  });
}
...
test('navigation between core pages works', async ({ page }) => {
  ...
  if (linkCount > 0) {
    ...
    test.skip(true, 'No navigation link to /projects found — layout may use collapsed sidebar');
  }
});
