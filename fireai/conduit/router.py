...
DESIGN LINEAGE:
  Adapted the grid-cell approach from fireai/core/cable_router.py
  (CableRouter._precompute_electrical_zones, A* implementation).
  Key difference: cable_router routes electrical wire with voltage-drop
  cost; this router routes physical conduit with NEC bend-radius cost.
  The two systems are complementary, not duplicate.

  OSE Piping Workbench uses FreeCAD geometry for obstacle detection;
  this implementation uses axis-aligned bounding boxes (AABB) consistent
  with the IFC building model already parsed by ifc_parser.py.

CONSTRAINTS:
  - Conduit OD clearance from obstacle surfaces: 25mm (NEC 300.4)
  - Electrical conduit separation: 300mm (NEC 760.24 / project spec)
  - No more than 360° cumulative bends (NEC 358.26/352.26/344.26)
...
# Discourages unnecessary direction changes, reflecting:
...
#  - gravity makes wire pulling harder on vertical runs
#  - EMT hangers / supports required every 3m vertical (NEC 358.30)
...
class ConduitRouter:
    ...
    This router is conduit-specific (not electrical cable). It provides
    the physical path; the fitting engine then places elbows and couplings.

    Algorithm lineage: adapted from CableRouter in fireai/core/cable_router.py,
    which itself implements grid-based A* with Manhattan heuristic. Key changes:
      - Cost function uses NEC bend penalty instead of voltage-drop penalty
      - Obstacle clearance uses conduit OD, not wire diameter
      - Cumulative bend limit enforced during search (360° NEC limit)
    ...
    def __init__(
        ...
        Args:
            obstacles:          List of bounding boxes to avoid.
                                Each is expanded by its clearance before routing.
        ...
        if not math.isfinite(grid_resolution_m) or grid_resolution_m <= 0.0:
            raise ValueError(
                f"grid_resolution_m={grid_resolution_m} must be positive finite. "
    ...
    def route(  # NOSONAR — S3776: cognitive complexity is inherent to the safety-critical algorithm
        ...
        _trade_size: TradeSize = TradeSize.HALF,  # NOSONAR — S1172: parameter retained for API stability
        ...
        # g_costs: grid_key → best g_cost seen so far
        ...
        while open_heap and iterations < _MAX_ITERATIONS:
            ...
            for dx, dy, dz in self._DIRECTIONS:
                ...
                # Step cost = Euclidean step length (always = grid_res for orthogonal)
                step_cost = self._res
                ...
                # NEC 360° cumulative bend limit — prune paths that exceed it
                # Each grid-level bend = 90°; 360°/90° = 4 bends max
                # (360 / 90 = 4 quarter-bends)
                if new_bends > 4:
                    # Path needs a pull box — handled by fitting_engine
                    # Still allow path to continue; pull box is inserted later
                    pass
        ...
        node: _AStarNode | None = found  # type: ignore[no-redef]
...
def _simplify_waypoints(waypoints: list[Point3D]) -> list[Point3D]:
    ...
    representation cleaner for fitting placement.
    ...
    for i in range(1, len(waypoints) - 1):
        ...
        # Only add curr if direction changes (i.e., it's a turn point)
        if d1 != d2:
            result.append(curr)
...
def orthogonal_astar(
    ...
    Args:
        ...
        grid_resolution_m: Grid step size (metres). Default 0.10.
