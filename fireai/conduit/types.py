...
Adapted from open-source MEP approaches (OpenMEP pattern: typed enums +
frozen dataclasses; OSE Piping: CSV-backed dimensional catalog concept)
but reimplemented for NFPA 72 fire alarm conduit installations and
integrated with the FireAI rules engine.
...
class ConduitType(enum.Enum):
    ...
    EMT      — Electrical Metallic Tubing (NEC Article 358)
                ...
                Standard for interior fire alarm wiring, NFPA 72 §12.2.

    UPVC_SCH40 — Rigid PVC Schedule 40 (NEC Article 352)
                 Lighter wall, lower pressure rating.
                 Used above ground in dry/damp locations.

    UPVC_SCH80 — Rigid PVC Schedule 80 (NEC Article 352)
                 Thicker wall, stronger, same OD as Sch 40.
                 Required in exposed/wet locations.
...
class FillResult:
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"{self.fill_percentage:.2f}%/{self.max_allowed_pct:.0f}% {status})"
        )
