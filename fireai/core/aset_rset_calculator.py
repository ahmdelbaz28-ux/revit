...
def calculate_rset(
    ...
    # V48 FIX: When detection_time_s is None, default to 60s (conservative estimate
    # for ceiling-mounted smoke detector) instead of 0.0. Even the fastest detector
    # has non-zero response time. Emit WARNING to encourage explicit specification.
    if detection_time_s is not None:
        dt = detection_time_s
    else:
        import logging as _log
        _log.getLogger(__name__).warning(
            "ASET-RSET-002: detection_time_s not provided — using conservative "
            "default of 60s (typical ceiling smoke detector response per NFPA 72 §17.6). "
            "Provide explicit detection time for accurate RSET calculation."
        )
        dt = 60.0
