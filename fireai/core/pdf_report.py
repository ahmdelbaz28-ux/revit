...
def _header_footer(canvas, doc) -> None:
    ...
    canvas.drawString(
        MARGIN + 4, h - MARGIN - 0.38 * cm, "FireAI  -  NFPA 72-2022 Compliance Report"
    )
    canvas.setFont("Helvetica", 7)
    canvas.drawRightString(
        w - MARGIN - 4, h - MARGIN - 0.38 * cm, f"CONFIDENTIAL - Page {doc.page}"
    )
    # Footer line
...
def _cover_page(report: BuildingReport) -> list:
    ...
    elems.append(
        Paragraph(
            "FireAI",
            ParagraphStyle(
                "logo", fontName="Helvetica-Bold", fontSize=36, textColor=C_BLUE_DARK, spaceAfter=2
            ),
        )
    )
    ...
    for label, val in meta:
        elems.append(
            Paragraph(f'<font name="Helvetica-Bold">{label}:</font>  {val}', STYLES["CoverSub"])
        )
    ...
    if report.non_compliant_floors:
        ...
        elems.append(
            Paragraph(
                ...
                ParagraphStyle(
                    "NCF", fontName="Helvetica", fontSize=9, textColor=C_RED, spaceAfter=4
                ),
            )
        )
...
def _scenario_section(scenario_results: dict[str, ScenarioBatteryResult]) -> list:
    ...
    for room_id, battery in scenario_results.items():
        sc_count = getattr(
            battery,
            "total_scenarios",
            getattr(battery, "scenario_count", len(getattr(battery, "results", []))),
        )
        sc_pass = getattr(battery, "passed", getattr(battery, "pass_count", "-"))
        sc_fail = getattr(
            battery,
            "failed",
            getattr(battery, "fail_count", getattr(battery, "scenario_fail_count", "-")),
        )
        worst = getattr(
            battery, "worst_detection_time", getattr(battery, "worst_detection_time_s", "-")
        )
        blind = getattr(battery, "blind_spots", getattr(battery, "total_blind_spots", []))
        blind_str = (
            "; ".join(str(b) for b in (blind if isinstance(blind, list) else []))
            if blind
            else "None"
        )
...
def _duct_section(report: BuildingReport) -> list:
    ...
    elems.append(
        Paragraph(
            f"Total duct detectors placed: <b>{report.total_duct_devices}</b>", STYLES["Body"]
        )
    )
