...
class AuditInput(BaseModel):
    ...
    zone: str = Field(
        description="Zone classification string (e.g., 'ZONE_1', 'ZONE_2')"
    )
    min_redundancy: int = Field(
...
class SafetyAuditEngine:
    ...
    def run_audit(
        ...
        try:
            ...
            violations.extend(v); total_checks += tc; passed_checks += pc
        except Exception as e:
            violations.append(AuditViolation(gate="REDUNDANCY", severity="CRITICAL",
                code="AUDIT-001", message=f"Redundancy gate failed: {e}",
                standard_ref="Safety audit integrity", remediation="Review input data"))
            total_checks += 1
        ...
        try:
            ...
            violations.append(AuditViolation(gate="FOULING", severity="CRITICAL",
                code="AUDIT-002", message=f"Fouling gate failed: {e}",
                standard_ref="Safety audit integrity", remediation="Review input data"))
            total_checks += 1
        ...
        try:
            ...
            violations.append(AuditViolation(gate="ZONE_MAPPING", severity="CRITICAL",
                code="AUDIT-003", message=f"Zone mapping gate failed: {e}",
                standard_ref="Safety audit integrity", remediation="Review input data"))
            total_checks += 1
        
        # ── Gate 4: Z-Axis / Vapor Density ──
        try:
            v, tc, pc = self._check_z_axis(
                substance, detector_z_positions, ceiling_height_m
            )
            violations.extend(v); total_checks += tc; passed_checks += pc
        except Exception as e:
            violations.append(AuditViolation(gate="Z_AXIS", severity="CRITICAL",
                code="AUDIT-004", message=f"Z-axis gate failed: {e}",
                standard_ref="Safety audit integrity", remediation="Review input data"))
            total_checks += 1
        ...
        try:
            ...
            violations.append(AuditViolation(gate="MENA", severity="CRITICAL",
                code="AUDIT-005", message=f"MENA gate failed: {e}",
                standard_ref="Safety audit integrity", remediation="Review input data"))
            total_checks += 1
    ...
    def _check_zone_mapping(
        ...
        if zone in _GAS_ZONES and hazard_type == HazardType.DUST:
            violations.append(AuditViolation(
                ...
                remediation=(
                    "Re-classify using correct zone type for DUST hazard "
                    "(Zone 20/21/22 per IEC 60079-10-2)."
                ),
            ))
            ...
            violations.append(AuditViolation(
                ...
                standard_ref="IEC 60079-10-1:2015 §1.3",
                remediation=(
                    "Re-classify using correct zone type for GAS hazard "
                    "(Zone 0/1/2 per IEC 60079-10-1)."
                ),
            ))
            ...
            violations.append(AuditViolation(
                ...
                remediation=(
                    "Perform separate dust zone classification per "
                    "IEC 60079-10-2 and apply most severe zone."
                ),
            ))
            ...
            violations.append(AuditViolation(
                ...
                standard_ref="IEC 60079-10-1:2015 §5.3",
                remediation=(
                    "Perform separate gas zone classification per "
                    "IEC 60079-10-1 and apply most severe zone."
                ),
            ))
            ...
            violations.append(AuditViolation(
                ...
                standard_ref="IEC 60079-10-1:2015 §1.3",
                remediation=(
                    "Provide zone classification and hazard type "
                    "to enable zone mapping verification."
                ),
            ))
        else:
            passed_checks = 1
