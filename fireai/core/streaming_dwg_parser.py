...
def _assemble_closed_polygons_v29(
    ...
    for idx, (start, end) in enumerate(lines):
        sx, sy = start
        ex, ey = end
        cs = (int(math.floor(sx / cell_size)),
              int(math.floor(sy / cell_size)))
        ce = (int(math.floor(ex / cell_size)),
              int(math.floor(ey / cell_size)))
...
class StreamingDXFParser:
    ...
    def stream_file(
        self, filepath: str
    ) -> Generator[StreamedRoom, None, StreamingStats]:
        """
        ...
        try:
            with open(filepath, "r", encoding="utf-8", errors="replace",
                      buffering=131_072) as fh:   # 128KB read buffer
                chunk_buf: List[str] = []
                for raw_line in fh:
        ...
        while i < n - 1:
            ...
            if code == "0" and val.upper() == "LINE":
                ...
                while j < min(i + 20, n - 1):
                    ...
                    gv  = lines[j+1].strip()
                    try:
                        if gc == "10":   x1 = float(gv) * s
                        elif gc == "20": y1 = float(gv) * s
                        elif gc == "11": x2 = float(gv) * s
                        elif gc == "21": y2 = float(gv) * s
                        elif gc == "0":  break   # Next entity
                    except ValueError:
                        pass
                    j += 2
    ...
    def process_batch(
        ...
        try:
            ...
            with ctx.Pool(processes=self.n_workers) as pool:
                results = pool.map(optimize_fn, rooms,
