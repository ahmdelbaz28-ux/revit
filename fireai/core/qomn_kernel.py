...
def _guard_finite(value: float, field: str) -> float:
    ...
    if not isinstance(value, (int, float)):
        raise PhysicsGuardError(
            field, value, "must be numeric", "IEEE-754-2008 §7"
        )
    if math.isnan(value):
        raise PhysicsGuardError(
            field, "NaN", "NaN is not permitted in safety-critical computation",
            "IEEE-754-2008 §7.2"
        )
    if math.isinf(value):
        raise PhysicsGuardError(
            field, "Inf", "Infinity is not permitted in safety-critical computation",
            "IEEE-754-2008 §7.4"
        )
    return float(value)
...
def guard_area_m2(area_m2: float) -> float:
    ...
    if v <= 0:
        raise PhysicsGuardError(
            "area_m2", v, "area must be > 0 m²", "Physics"
        )
    if v > NFPA_MAX_M2:
        raise PhysicsGuardError(
            "area_m2", f"{v:.2f}", f"exceeds NFPA 72 max 232.26 m² (2500 ft²) per detector",
...
def guard_ceiling_height_m(h: float) -> float:
    ...
    NFPA_MAX_M = 18.288   # 60 ft × 0.3048
    if v <= 0:
        raise PhysicsGuardError(
            "ceiling_height_m", v, "ceiling height must be > 0", "Physics"
        )
    if v > NFPA_MAX_M:
        raise PhysicsGuardError(
            "ceiling_height_m", f"{v:.2f}m",
            f"exceeds NFPA 72 §17.7.3.2.4 maximum 18.288m (60ft). "
...
def guard_current_a(current: float, wire_ampacity: float, gauge: str) -> float:
    ...
    if v > wa:
        raise PhysicsGuardError(
            "current_a", f"{v:.3f}A",
            f"exceeds AWG {gauge} ampacity {wa:.1f}A",
            "NEC 2023 §310.16"
        )
    return v
...
def guard_voltage_v(voltage: float, system_rating: float) -> float:
    ...
    if v < 0:
        raise PhysicsGuardError(
            "voltage_v", v, "voltage cannot be negative", "Physics"
        )
    if v > system_rating:
        raise PhysicsGuardError(
            "voltage_v", f"{v:.2f}V",
...
def guard_efficiency(eff: float) -> float:
    ...
    if v <= 0:
        raise PhysicsGuardError(
            "efficiency", v, "efficiency must be > 0", "Physics"
        )
    if v > 1.0:
        raise PhysicsGuardError(
            "efficiency", v,
            "efficiency > 1.0 (100%) violates conservation of energy",
            "Physics"
        )
    return v
...
NEC_AMPACITY_60C: Dict[str, float] = {
    "18": 7.0, "16": 13.0, "14": 15.0, "12": 20.0, "10": 30.0,
    "8": 40.0, "6": 55.0, "4": 70.0, "2": 95.0, "1": 110.0,
    "1/0": 125.0, "2/0": 145.0, "3/0": 165.0, "4/0": 195.0,
}
...
def compute_smoke_detector_spacing(ceiling_height_m: float) -> Dict[str, Any]:
    ...
    if spacing_m is None:
        ...
        raise PhysicsGuardError(
            "ceiling_height_m", h,
            "No table row found — ceiling exceeds NFPA 72 scope",
            "NFPA 72-2022 Table 17.6.3.1"
        )
...
def compute_heat_detector_spacing(
    ...
    return {
        ...
        "coverage_radius_m":  round(radius_m, 6),
        "max_spacing_m":      NFPA72_HEAT_MAX_SPACING_M,
        "is_within_max":      spacing_m <= NFPA72_HEAT_MAX_SPACING_M,
...
def compute_battery_capacity_ah(
    ...
    *,
    ...
    return {
        ...
        "formula": (
            f"Ah = (({i_sb}A×{standby_hours}h + {i_al}A×{alarm_hours:.4f}h) "
            f"/ {eff}) × {sf} = {ah_required:.4f}Ah"
        ),
        "computation_hash":   result_hash,
    }
...
def compute_voltage_drop(
    ...
    if i < 0: raise PhysicsGuardError("current_a", i, "cannot be negative", "Physics")
    if L <= 0: raise PhysicsGuardError("length_m", L, "must be > 0", "Physics")
    if v <= 0: raise PhysicsGuardError("supply_voltage_v", v, "must be > 0", "Physics")

    awg = awg_gauge.strip().upper().replace("AWG", "").strip()
    if awg not in NEC_TABLE8_RESISTANCE_OHM_PER_KM:
        raise ValueError(
            f"AWG gauge '{awg_gauge}' not in NEC Table 8. "
            f"Valid: {sorted(NEC_TABLE8_RESISTANCE_OHM_PER_KM.keys())}"
        )
...
def validate_voltage_drop_result(result: Dict) -> Dict:
    ...
    if vd >= result["supply_voltage_v"]:
        raise ValidationError(
            f"Voltage drop {vd:.4f}V ≥ supply {result['supply_voltage_v']}V — "
