...
class ComplianceProofDocument:
    ...
    def __init__(
        ...
        self.generation_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    def _room_summary_table(self) -> str:
        ...
        for i, rec in enumerate(self.records, 1):
            ...
            lines.append(
                f"| {i} | {room.name} | "
                f"{room.width:.1f} × {room.length:.1f} | "
                f"{room.ceiling_height:.1f} | "
                f"{layout.count} | "
                f"{layout.coverage_pct:.1f}% | "
    ...
    def _detailed_room_results(self) -> str:
        ...
        for i, rec in enumerate(self.records, 1):
            ...
            lines.extend([
                ...
                "",
                f"**Dimensions:** {room.width:.1f} m × {room.length:.1f} m "
                f"(Area: {room.width * room.length:.1f} m²)",
                f"**Ceiling Height:** {room.ceiling_height:.1f} m",
                f"**Coverage Radius Used:** {layout.coverage_radius:.2f} m "
                ...
                f"**Efficiency Ratio:** {layout.efficiency_ratio:.2f}",
                f"**Coverage:** {layout.coverage_pct:.2f}%",
