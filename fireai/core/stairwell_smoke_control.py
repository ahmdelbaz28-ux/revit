...
class StairwellSmokeControlIntegrator:
    ...
    def generate_active_smoke_defense(
        ...
        # V51 FIX: NaN building_height disables all pressurization.
        # NaN > 22.86 = False, so pressurization_required = False.
        # ALL stairwell smoke control checks are silently skipped.
        # A 50-story building would have zero pressurization analysis.
        import math as _height_math
        if bldg_height is not None and (_height_math.isnan(bldg_height) or _height_math.isinf(bldg_height)):
            desc = (
                f"STAIRWELL-002: building_height_m={bldg_height} (NaN or Inf). "
                f"All pressurization analysis is DISABLED. Stairwell smoke "
                f"control cannot be verified per NFPA 92 §6.1. This is a "
                f"data integrity error — provide a valid building height."
            )
        ...
        # V51 FIX: NaN/Inf building_height — pressurization_required would be
        # False (NaN > 22.86 = False), disabling ALL smoke control checks.
        # For NaN/Inf, force pressurization_required=True (conservative/fail-safe).
        # For 0.0 or negative, keep pressurization_required=False (V48 CRITICAL log
        # already warns that the module is inactive — this is the design intent:
        # height=0.0 means "not provided", not "literally zero metres").
        try:
            height_is_nan_or_inf = (
                bldg_height is not None
                and not _height_math.isnan(bldg_height)
                and not _height_math.isinf(bldg_height)
            ) is False and bldg_height is not None
        except (TypeError, ValueError):
            height_is_nan_or_inf = True

        if height_is_nan_or_inf and bldg_height is not None and not (_height_math.isnan(bldg_height) or _height_math.isinf(bldg_height)):
            # Normal numeric value
            pressurization_required = bldg_height > self.min_height_m
        elif bldg_height is not None and (_height_math.isnan(bldg_height) or _height_math.isinf(bldg_height)):
            # NaN or Inf — assume pressurization required (conservative)
            pressurization_required = True
        else:
            # None, 0.0, or negative — module inactive (V48 log warns)
            pressurization_required = bldg_height is not None and bldg_height > self.min_height_m
        ...
        if pressurization_required:
            ...
            for stair in stairwells:
                ...
                # V50 FIX: Validate design_pressure for NaN and None.
                # NaN passes both min and max checks silently (NaN < 25 = False,
                # NaN > 85 = False). Missing pressure when pressurization is
                # required means the system cannot verify compliance at all.
                import math as _math
                if design_pressure is not None and (_math.isnan(design_pressure) or _math.isinf(design_pressure)):
                    desc = (
                        f"Stairwell '{name}' ({zone_id}) design_pressure is "
                        f"NaN/Inf ({design_pressure}). Cannot validate against "
                        f"NFPA 92 §6.4 (25–85 Pa). This is a data integrity "
                        f"error — pressurization compliance CANNOT be verified."
                    )
                    if Violation is not None:
                        violations.append(Violation(
                            severity="CRITICAL",
                ...
                # V50 FIX / V51 REFINEMENT: Missing design_pressure when pressurization is required.
                # Without pressure data, the system cannot verify that the stairwell
                # meets NFPA 92 §6.4 (25–85 Pa).
                # V51 REFINEMENT: When pressurization EQUIPMENT is present (fan + switches),
                # the stairwell is mechanically equipped — the missing design_pressure is a
                # commissioning data gap (WARNING), not a design deficiency (CRITICAL).
                # When NO equipment is present, missing design_pressure is CRITICAL because
                # there is no pressurization system at all.
                # This distinction is important: a stairwell with a fan but no pressure
                # data has equipment that CAN provide pressurization; the issue is that
                # we cannot verify the exact operating pressure. A stairwell without a fan
                # has NO pressurization capability at all.
                if design_pressure is None and pressurization_required:
                    has_fan = stair.get("has_pressurization_fan", False)
                    has_switches = stair.get("has_pressure_switches", False)
                    has_equipment = has_fan and has_switches

                    if has_equipment:
                        # Equipment present but commissioning data missing → WARNING
                        # Not unsafe because the system can function, just unverified
                        desc = (
                            f"Stairwell '{name}' ({zone_id}) has pressurization "
                            f"equipment (fan + pressure switches) but design_pressure_pa "
                            f"is NOT specified. Cannot verify compliance with NFPA 92 "
                            f"§6.4 (25–85 Pa). Commissioning data should be provided "
                            f"for verification. [NFPA 92-2024 §6.4]"
                        )
                        logger.warning(desc)
                    else:
                        # No equipment or partial equipment → CRITICAL
                        desc = (
                            f"Stairwell '{name}' ({zone_id}) requires pressurization "
                            f"but design_pressure_pa is NOT specified. Cannot verify "
                            f"compliance with NFPA 92 §6.4 (25–85 Pa). "
                            f"Pressurization design MUST be validated by a licensed "
                            f"fire protection engineer. [NFPA 92-2024 §6.4]"
                        )
                        if Violation is not None:
                            violations.append(Violation(
                                severity="CRITICAL",
