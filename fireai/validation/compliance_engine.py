...
class ComplianceEngine:
        ...
        return [
            ...
            ComplianceRule(
                ...
                validator=lambda ctx: (
                    ctx.get("spacing_m", float("inf"))
                    <= ctx.get("max_spacing_for_height", 0) * 1.001
                ),
            ...
            ComplianceRule(
                ...
                validator=lambda ctx: (
                    ctx.get("ceiling_type") != "sloped" or ctx.get("spacing_m", float("inf")) <= 6.4
                ),
            ...
            ComplianceRule(
                ...
                validator=lambda ctx: ctx.get("wall_distance_m", 0) >= 0.1016,
            ...
            ComplianceRule(
                ...
                validator=lambda ctx: (
                    ctx.get("v_drop_total_percent", ctx.get("v_drop_percent", 100)) <= 5.0
                ),
            ...
            ComplianceRule(
                ...
                validator=lambda ctx: (
                    ctx.get("design_pressure_pa", 100) >= 25.0
                    if ctx.get("pressurization_required", False)
                    else True
                ),
            ...
            ComplianceRule(
                ...
                validator=lambda ctx: (
                    ctx.get("release_grade") != "CONTINUOUS" or ctx.get("zone_delta", 0) <= 0
                ),
    ...
    def validate_and_report(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "compliance_percentage": round((1 - len(violations) / len(self.rules)) * 100, 1)
            if self.rules
            else 100.0,
        }
