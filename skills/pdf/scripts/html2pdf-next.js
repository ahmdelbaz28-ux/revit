...
function loadPlaywright() {
	...
	try {
		...
		console.error("[html2pdf] Error loading playwright:", err.message || err);
	}
	...
	try {
		...
		console.error(
			"[html2pdf] Error finding npm global root:",
			err.message || err,
		);
	}
	...
	for (const base of roots) {
		...
		try {
			return Module.createRequire(pkg)("playwright");
		} catch (err) {
			console.error(
				"[html2pdf] Error loading playwright from path:",
				err.message || err,
			);
		}
	}
...
function loadPdfLib() {
	try {
		...
		console.error("[html2pdf] Error loading pdf-lib:", err.message || err);
	}
	const Module = require("node:module");
	try {
		const g = execSync("npm root -g", { stdio: ["ignore", "pipe", "ignore"] })  // NOSONAR - javascript:S4036
		...
		console.error(
			"[html2pdf] Error loading pdf-lib from global path:",
			err.message || err,
		);
	}
	...
	if (allowInstall) {
		...
		if (r.status === 0) {
			try {
				...
				console.error(
					"[html2pdf] Error getting Chromium executable path after install:",
					err.message || err,
				);
			}
			if (exe && fs.existsSync(exe))
				return { status: "installed", executablePath: exe };
		}
	}
...
async function preRenderHooks(page) {  // NOSONAR - javascript:S3776
	...
	if (!katexStatus.lib && katexStatus.raw && !katexStatus.rendered) {
		console.log(
			"  ⏳ Auto-injecting KaTeX CDN (math formulas detected but KaTeX not loaded)...",
		);
		...
		await page.addScriptTag({
			url: "https://cdn.jsdelivr.net/npm/katex@0.16.22/dist/contrib/auto-render.min.js",
		});
		await sleep(2000); // Wait for CDN scripts to load
		// Re-check
		const recheckLib = await page.evaluate(
		...
		if (recheckLib) {
			...
			console.log(
				"  ⚠ KaTeX CDN failed to load — math will render as raw text",
			);
			warnings.push(
				"KaTeX CDN injection failed; math formulas may appear as raw LaTeX code",
			);
		}
	}
	...
	if (nFixed) {
		console.log(
			`  ⚠ Fixed ${nFixed} oversized elements (removed break-inside: avoid)`,
		);
	}
	...
	if (vOverflowFix.fixes.length) {
		console.log(
			"  ⚠️  Removed fixed height + overflow:hidden — content will paginate naturally:",
		);
		vOverflowFix.fixes.forEach((f) => {
			console.log(
				`    ${f.selector}: was ${f.oldHeight}px with ${f.clipped}px clipped → now auto (content will flow to next page)`,
			);
		});
	}
	...
	if (absBottomFix.length) {
		console.log(
			"  ⚠️  Converted absolute-bottom elements to document flow (prevents overlap on multi-page):",
		);
		absBottomFix.forEach((f) => {
			console.log(
				`    ${f.selector}: was position:absolute;bottom:${f.bottom} → now static (flows with content)`,
			);
		});
	}
	...
	await page.evaluate(() => {
		...
		const hasPageRule = styles.some((s) =>
	...
	try {
		...
		if (continuousInfo) {
			...
			if (width || height) {
				...
				console.log(
					`   Custom size: ${pdfOpts.width || "auto"} × ${pdfOpts.height || "auto"}`,
				);
			} else {
				...
				if (pageSize) {
					...
					if (
						...
						measuredContentHeight > pageSize.height + 5
					) {
						const estPages = Math.ceil(measuredContentHeight / pageSize.height);
						console.log(
							`   Content height: ${measuredContentHeight}px → ~${estPages} pages`,
						);
					}
				} else {
					pdfOpts.format = "A4";
		...
		if (mergeFiles?.length && postResult.pageCount > postResult.originalPages) {
			console.log(
				`  Merged:  +${postResult.pageCount - postResult.originalPages} pages from ${mergeFiles.length} file(s)`,
			);
		}
