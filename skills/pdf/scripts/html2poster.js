...
async function main() {
	...
	if (bInfo.status === "missing") {
		console.error(
			"Error: No Chromium found. Run: npx playwright install chromium",
		);
		process.exit(1);
	}
	...
	try {
		...
		if (overflowFixed > 0) {
			console.log(
				`   ✓ Added overflow:hidden to ${overflowFixed} container(s)`,
			);
		}
		...
		const bgSync = await page.evaluate(() => {
			...
			const poster =
				document.querySelector(".poster") || document.querySelector(".page");
			if (poster) {
				const posterBg = getComputedStyle(poster).backgroundColor;
				if (
					posterBg &&
					posterBg !== "rgba(0, 0, 0, 0)" &&
					posterBg !== "transparent"
				) {
		...
		const measurement = await page.evaluate(() => {
			...
			return {
				...
				selector: poster.className
					? `.${poster.className.split(" ")[0]}`
					: poster.tagName,
			};
		});

		console.log(
			`   ✓ Measured: ${measurement.selector} = ${measurement.scrollWidth}×${measurement.scrollHeight}px`,
		);

		if (measurement.scrollWidth > widthPx + 2) {
			console.log(
				`   ⚠ WARNING: scrollWidth (${measurement.scrollWidth}px) > width (${widthPx}px)`,
			);
			console.log(
				`     Decorative elements may still overflow. Check for position:absolute elements with negative offsets.`,
			);
		}
		...
		if (contentHeight > maxHeight) {
			console.log(
				`   ⚠ Content height ${contentHeight}px exceeds max ${maxHeight}px, clamping.`,
			);
			contentHeight = maxHeight;
		}
		if (contentHeight < 100) {
			console.log(
				`   ⚠ Content height ${contentHeight}px seems too small, using 960px fallback.`,
			);
			contentHeight = 960;
		}
		...
		console.log(`\n   ✅ Done: ${absOut}`);
		console.log(
			`      Size: ${(fs.statSync(absOut).size / 1024).toFixed(1)} KB`,
		);
	} finally {
		await browser.close();
	}
