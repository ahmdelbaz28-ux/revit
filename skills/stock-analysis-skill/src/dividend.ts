...
import type { DividendAnalysis, IncomeRating, PayoutStatus } from "./types";
...
function calcSafetyScore(data: {
	...
	if (data.payoutRatio !== null) {
		if (data.payoutRatio < 40) {
			score += 20;
			factors.push(`派息率健康（${data.payoutRatio.toFixed(0)}%）`);
		} else if (data.payoutRatio < 60) {
			score += 10;
			factors.push(`派息率适中（${data.payoutRatio.toFixed(0)}%）`);
		} else if (data.payoutRatio < 80) {
			score -= 10;
			factors.push(`派息率偏高（${data.payoutRatio.toFixed(0)}%）`);
		} else {
			score -= 20;
			factors.push(`派息率不可持续（${data.payoutRatio.toFixed(0)}%）`);
		}
	}

	// 5年增长率（±15）
	if (data.dividendGrowth5y !== null) {
		if (data.dividendGrowth5y > 10) {
			score += 15;
			factors.push(`股息增长强劲（${data.dividendGrowth5y.toFixed(1)}% CAGR）`);
		} else if (data.dividendGrowth5y > 5) {
			score += 10;
			factors.push(`股息增长良好（${data.dividendGrowth5y.toFixed(1)}% CAGR）`);
		} else if (data.dividendGrowth5y > 0) {
			score += 5;
			factors.push(`股息小幅增长（${data.dividendGrowth5y.toFixed(1)}% CAGR）`);
		} else {
			score -= 15;
			factors.push(`股息下降（${data.dividendGrowth5y.toFixed(1)}% CAGR）`);
		}
	}

	// 连续增长年数（±15）
	if (data.consecutiveYears !== null) {
		if (data.consecutiveYears >= 25) {
			score += 15;
			factors.push(`股息贵族（连续${data.consecutiveYears}年增长）`);
		} else if (data.consecutiveYears >= 10) {
			score += 10;
			factors.push(`长期稳定股息（${data.consecutiveYears}年）`);
		} else if (data.consecutiveYears >= 5) {
			score += 5;
			factors.push(`股息稳定（${data.consecutiveYears}年）`);
		}
	}

	// 高收益率风险（-10）
	if (data.dividendYield !== null) {
		if (data.dividendYield > 8) {
			score -= 10;
			factors.push(
				`收益率过高（${data.dividendYield.toFixed(1)}%），需核实可持续性`,
			);
		} else if (data.dividendYield < 1) {
			factors.push(`收益率偏低（${data.dividendYield.toFixed(2)}%）`);
		}
	}
...
export async function analyzeDividend(
	...
	try {
		...
		return {
			...
			safetyScore: 0,
			safetyFactors: [`数据获取失败：${err.message}`],
			incomeRating: "poor",
			dividendHistory: [],
	...
	if (!raw.annualDividend || raw.annualDividend === 0) {
		return {
			...
			annualDividend: null,
			payoutRatio: null,
			payoutStatus: "no_dividend",
			dividendGrowth5y: null,
			consecutiveYears: null,
			exDividendDate: null,
			paymentFrequency: null,
			safetyScore: 0,
			safetyFactors: ["该股票不派息"],
			incomeRating: "no_dividend",
			dividendHistory: [],
	...
	// 生成摘要
	const parts: string[] = [];
	if (raw.dividendYield)
		parts.push(`收益率 ${Number(raw.dividendYield).toFixed(2)}%`);
	if (payoutRatio) parts.push(`派息率 ${payoutRatio.toFixed(0)}%`);
	if (raw.dividendGrowth5y)
		parts.push(
			`5年增长 ${Number(raw.dividendGrowth5y) > 0 ? "+" : ""}${Number(raw.dividendGrowth5y).toFixed(1)}%`,
		);
	if (raw.consecutiveYears && raw.consecutiveYears >= 5)
		parts.push(`连续增长 ${raw.consecutiveYears} 年`);
	...
	return {
		...
		dividendYield: raw.dividendYield
			? Number(Number(raw.dividendYield).toFixed(2))
			: null,
		annualDividend: raw.annualDividend ?? null,
		payoutRatio,
		payoutStatus,
		dividendGrowth5y: raw.dividendGrowth5y
			? Number(Number(raw.dividendGrowth5y).toFixed(2))
			: null,
		consecutiveYears: raw.consecutiveYears ?? null,
...
`;
	...
	if (analysis.dividendHistory.length > 0) {
		md += `\n近年股息历史：\n`;
		md += analysis.dividendHistory
			.slice(0, 5)
			.map((h) => `- ${h.year}年：$${h.total}`)
			.join("\n");
	}
