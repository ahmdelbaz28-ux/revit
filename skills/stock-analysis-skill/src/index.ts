...
function parseInput(raw: unknown): {
	...
	if (typeof raw === "string") {
		raw
			...
			.filter(Boolean)
			.forEach((c) => stocks.push(c));
	} else if (typeof raw === "object" && raw !== null) {
		const input = raw as any;
		const rawStocks: (string | StockInput)[] =
			input.stocks ?? (input.stock ? [input.stock] : []);
...
export async function run(rawInput: unknown): Promise<{
	...
	await Promise.all([
		includeGlobalMacro
			? fetchGlobalMacro().then((r) => {
					globalMacro = r;
				})
			: Promise.resolve(),
		includeMarketReview
			? fetchMarketOverview().then((r) => {
					marketOverview = r;
				})
			: Promise.resolve(),
	]);
	...
	try {
		...
		log(
			`数据完成：${stockDataList.filter((d) => !d.error).length}/${stocks.length}`,
		);
	} catch (err: any) {
		return {
			success: false,
			format: outputFormat,
			content: `❌ 数据抓取失败：${err.message}`,
			summary: {
				total: stocks.length,
				buy: 0,
				watch: 0,
	...
	// LLM 分析
	const analysisResults = await analyzeMultipleStocks(
		stockDataList,
		outputFormat,
		positions,
		includeDividend,
	...
	try {
		...
		if (outputFormat === "pdf") content = await exportToFormat(markdown, "pdf");
			...
			content = await exportToFormat(markdown, "word");
		else content = markdown;
	} catch (err: any) {
		return {
			success: false,
			format: outputFormat,
			content: `❌ 报告生成失败：${err.message}`,
			summary: {
				total: stocks.length,
				buy: 0,
				watch: 0,
	...
	const summary = {
		...
		buy: analysisResults.filter((r) => ["买入", "强烈买入"].includes(r.verdict))
			.length,
...
export async function runDividend(tickers: string | string[]): Promise<string> {
	const codes = Array.isArray(tickers)
		? tickers
