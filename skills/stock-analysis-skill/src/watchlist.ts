...
import type { Market, Verdict, WatchlistAlert, WatchlistItem } from "./types";
...
export async function addToWatchlist(
	...
	if (currentPrice === null) {
		return {
			success: false,
			action: "error",
			message: `无法获取 ${ticker} 的价格，请确认代码正确`,
		};
	}
	...
	if (existingIdx >= 0) {
		...
		if (opts.alertOnSignal !== undefined)
			existing.alertOnSignal = opts.alertOnSignal;
...
export function formatAlertsMarkdown(
	result: Awaited<ReturnType<typeof checkAlerts>>,
): string {
	if (result.count === 0) {
		return `## 🔔 自选股提醒检查\n\n_当前没有触发任何提醒。_\n`;
	}
