...
import type { RumorItem, RumorScanResult, RumorType } from "./types";
...
function calcImpactScore(
	...
	switch (type) {
		case "ma":
			score += 5;
			reasons.push("并购/收购类传闻，市场冲击最大");
			break;
		case "insider":
			score += 4;
			reasons.push("内部人交易信号，可能预示重大动向");
			break;
		case "analyst":
			score += 3;
			reasons.push("分析师评级调整，影响机构定价");
			break;
		case "regulatory":
			score += 3;
			reasons.push("监管动态，直接影响经营合规性");
			break;
		case "earnings":
			score += 2;
			reasons.push("业绩预期变动");
...
async function extractRumors(_zai: any, rawNews: string): Promise<RumorItem[]> {
	try {
		const parsed: any[] = JSON.parse(
			rawNews.replace(/```json|```/g, "").trim(),
		);
...
`;
	...
	if (result.topTickers.length > 0) {
		md += `### 📊 传闻热度排行\n`;
		md += result.topTickers
			.map((t) => `- $${t.ticker}：被提及 ${t.count} 次`)
			.join("\n");
		md += "\n";
	}
