...
const COMPENSATION = {
	HEADING_WIDTH: 0.25,
	SINGLE_LINE_NARROW: 0.18,
	SINGLE_LINE_NORMAL: 0.1,
	MULTI_LINE: 0.05,
	SHORT_TEXT_EXTRA: 0.12,
	NUMERIC_TEXT_EXTRA: 0.08,
	NOWRAP_EXTRA: 0.15,
	MAX_WIDTH_FACTOR: 0.4,
	TEXT_HEIGHT: 0.08,
	LIST_HEIGHT: 0.06,
	MIN_FONT_SIZE_PT: 11,
	MAX_CHARS_CJK: 350,
	MAX_CHARS_LATIN: 550,
	VERTICAL_BALANCE_THRESHOLD: 0.55,
	OVERLAP_TOLERANCE_IN: 0.05,
	BOUNDS_TOLERANCE_IN: 0.02,
...
function extractTextContent(el) {
	...
	if (Array.isArray(el.items))
		return el.items.map((r) => r.text || "").join("");
	return "";
}
...
function getAdjustedTextPosition(el, slideWidthIn) {
	...
	const hComp =
		el.type === "list" ? COMPENSATION.LIST_HEIGHT : COMPENSATION.TEXT_HEIGHT;
	const finalW = Math.min(adjustedW, MAX_TEXT_WIDTH_IN);
	return {
		x: adjustedX,
		y: el.position.y,
		w: finalW,
		h: el.position.h * (1 + hComp),
	};
}
function checkElementBounds(slideData, sw, sh) {
	const w = [];
	const tol = COMPENSATION.BOUNDS_TOLERANCE_IN;
	const textTypes = new Set(["p", "h1", "h2", "h3", "h4", "h5", "h6", "list"]);
	for (const el of slideData.elements) {
		if (!el.position) continue;
		const p = textTypes.has(el.type)
			? getAdjustedTextPosition(el, sw)
			: el.position;
		if (p.x < -tol)
			w.push(
				`⚠ BOUNDS: "${getElementLabel(el)}" extends ${(-p.x * 72).toFixed(0)}pt beyond LEFT`,
			);
		if (p.y < -tol)
			w.push(
				`⚠ BOUNDS: "${getElementLabel(el)}" extends ${(-p.y * 72).toFixed(0)}pt beyond TOP`,
			);
		if (p.x + p.w > sw + tol)
			w.push(
				`⚠ BOUNDS: "${getElementLabel(el)}" extends ${((p.x + p.w - sw) * 72).toFixed(0)}pt beyond RIGHT`,
			);
		if (p.y + p.h > sh + tol)
			w.push(
				`⚠ BOUNDS: "${getElementLabel(el)}" extends ${((p.y + p.h - sh) * 72).toFixed(0)}pt beyond BOTTOM`,
			);
	}
	return w;
}
function checkVerticalBalance(slideData, sh) {
	const ce = slideData.elements.filter((e) =>
		["p", "h1", "h2", "h3", "h4", "h5", "h6", "list"].includes(e.type),
	);
	if (!ce.length) return [];
...
function checkTextOverlaps(slideData) {
	const w = [];
	const te = slideData.elements.filter((e) =>
		["p", "h1", "h2", "h3", "h4", "h5", "h6", "list"].includes(e.type),
	);
	for (let i = 0; i < te.length; i++)
		for (let j = i + 1; j < te.length; j++) {
			const a = te[i].position,
				b = te[j].position,
				tol = COMPENSATION.OVERLAP_TOLERANCE_IN;
			const ow = Math.min(a.x + a.w, b.x + b.w) - Math.max(a.x, b.x);
			const oh = Math.min(a.y + a.h, b.y + b.h) - Math.max(a.y, b.y);
			if (ow > tol && oh > tol)
				w.push(
					`⚠ OVERLAP: "${getElementLabel(te[i])}" overlaps "${getElementLabel(te[j])}"`,
				);
		}
	return w;
}
function checkMinFontSize(slideData) {
	...
	for (const el of slideData.elements) {
		...
		if (fs > 0 && fs < COMPENSATION.MIN_FONT_SIZE_PT)
			e.push(
				`Text "${getElementLabel(el)}" is ${fs.toFixed(1)}pt — min is ${COMPENSATION.MIN_FONT_SIZE_PT}pt`,
			);
	}
	return e;
}
function checkCharCount(slideData) {
	...
	return total > lim
		? [`⚠ DENSITY: ${total} chars (limit ${lim}) — split into multiple slides`]
		: [];
}
function calculateWidthCompensation(el, slideWidthIn) {
	...
	if (/^[\d\s.,\-/+%$#@!?:;()[\]]+$/.test(txt))
		f += COMPENSATION.NUMERIC_TEXT_EXTRA;
	...
	if (
		...
		single &&
		txt.length >= 10 &&
		txt.length < COMPENSATION.AUTO_SHORT_TEXT_THRESHOLD
	) {
...
function fixImageExtension(imagePath, tmpDir) {
	try {
		...
		if (
			...
			(currentExt === ".jpeg" && actualExt === ".jpg") ||
			(currentExt === ".jpg" && actualExt === ".jpeg")
		) {
			return imagePath;
		}
		...
		const fixedPath = path.join(
			tmpDir,
			path.basename(imagePath, currentExt) + actualExt,
		);
...
async function addBackground(slideData, targetSlide, pres, tmpDir) {
	if (slideData.background.type === "image" && slideData.background.path) {
		...
		const slideH = pres.presLayout
			? pres.presLayout.height / EMU_PER_IN
			: 5.625;
...
function addElements(slideData, targetSlide, pres, tmpDir) {
	const slideWidthIn = pres.presLayout
		...
		: 10;
	const slideHeightIn = pres.presLayout
		? pres.presLayout.height / EMU_PER_IN
		: 5.625;
	...
	for (const el of sortedElements) {
		if (el.type === "image") {
			...
			if (el.position.y + adjustedH > slideHeightIn)
				adjustedH = slideHeightIn - el.position.y;
			if (adjustedH < 0.1) adjustedH = 0.1;
			// Clamp list width to slide right edge
			const listX = el.position.x;
			...
			if (adjustedX + adjustedW > slideWidthIn)
				adjustedW = slideWidthIn - adjustedX;
...
async function extractSlideData(page) {
	return await page.evaluate(() => {
		...
		const FONT_FALLBACK_MAP = {
			...
			stsong: "SimSun",
			stfangsong: "FangSong",
			"apple sd gothic neo": "Microsoft YaHei",
			"noto sans sc": "Microsoft YaHei",
			"noto sans tc": "Microsoft YaHei",
			...
			"system-ui": "Century Gothic",
			"-apple-system": "Century Gothic",
			blinkmacsystemfont: "Century Gothic",
			...
			inter: "Century Gothic",
			roboto: "Arial",
			"roboto slab": "Rockwell",
			"open sans": "Century Gothic",
			lato: "Century Gothic",
			...
			"source sans 3": "Corbel",
			"source serif pro": "Georgia",
			"work sans": "Century Gothic",
			"dm sans": "Century Gothic",
			"space grotesk": "Century Gothic",
			"plus jakarta sans": "Candara",
			manrope: "Corbel",
			"fira sans": "Corbel",
		...
		const mapFontFace = (fontFamily, textContent = "") => {
			...
			for (const font of fonts) {
				...
				if (PPT_SAFE_FONTS.has(lower)) {
					...
					if (isCJKFont && !hasCJKChars(textContent))
						return _fc.latin || "Century Gothic";
					return font;
				}
				if (FONT_FALLBACK_MAP[lower]) {
					const mapped = FONT_FALLBACK_MAP[lower];
					const mappedIsCJK = CJK_FONT_FRAGMENTS.some((f) =>
						mapped.toLowerCase().includes(f),
					);
				...
				if (
					...
					continue;
				if (isCJKFont || hasCJKChars(textContent))
					return _fc.cjk || "Microsoft YaHei";
				...
				if (isText) {
					...
					if (
						...
						if (computed.letterSpacing && computed.letterSpacing !== "normal")
							options.charSpacing = pxToPoints(computed.letterSpacing);
		...
		const inlineAsBlockTags = new Set([
			"SPAN",
			"STRONG",
			"B",
			"EM",
		...
		document.querySelectorAll("*").forEach((el) => {
					...
					style: {
						...
						charSpacing:
							computed.letterSpacing && computed.letterSpacing !== "normal"
								? pxToPoints(computed.letterSpacing)
								: undefined,
						paraSpaceBefore: 0,
			...
			if (
				inlineAsBlockTags.has(el.tagName) &&
				!el.closest("p,h1,h2,h3,h4,h5,h6,li")
			) {
				el._treatAsP = true;
			} else if (
				leafBlockTags.has(el.tagName) &&
				!el.closest("p,h1,h2,h3,h4,h5,h6,li")
			) {
...
async function html2pptx(htmlFile, pres, options = {}) {
	...
	try {
		...
		if (allWarnings.length > 0) {
			const suggestions = allWarnings
				.map((w, i) => `  ${i + 1}. ${w}`)
				.join("\n");
			console.warn(
				`[html2pptx] ${htmlFile}: ${allWarnings.length} warning(s):\n${suggestions}`,
			);
		}
