 ...
 */
async function _createFromImage(imagePath: string) {
	try {
		const zai = await ZAI.create();
		...
		console.log(
			`Image converted to base64 (${base64Image.substring(0, 50)}...)`,
		);
...
async function query(zai: any, taskId: string) {
	try {
		...
		while (result.task_status === "PROCESSING" && pollCount < maxPolls) {
			pollCount++;
			console.log(
				`Poll ${pollCount}/${maxPolls}: Status is ${result.task_status}, waiting ${pollInterval / 1000}s...`,
			);
