"""
qomn_conduit.bend — NEC Bend Radius Verifier
=============================================
...
def verify_bend_radius(
    ...
    counts material by up to 57% for 90° bends.

    Args:
        conduit_type:   NEC wiring method.
        trade_size:     Nominal trade size.
        actual_radius:  Measured centreline bend radius (inches).
        angle_deg:      Bend angle in degrees. Default 90°.

    Returns:
        Result.ok(BendResult) — radius checked (is_compliant flag indicates status).
    ...
    if not math.isfinite(actual_radius):
        return Result.err(PhysicsError(
            message=f"actual_radius={actual_radius} is not finite.",
            remediation="Bend radius must be a positive finite number.",
        ))
    if actual_radius <= 0.0:
        return Result.err(PhysicsError(
            message=f"actual_radius={actual_radius} ≤ 0.",
            remediation="Bend radius must be > 0. A zero or negative radius is physically impossible.",
        ))
    ...
    if angle_deg > 360.0:
        return Result.err(PhysicsError(
            ...
            remediation="A single bend cannot exceed 360°. For spiral routing, sum multiple bend results.",
        ))
    ...
    if not is_compliant:
        return Result.err(CodeViolationError(
            message=(
                f"Bend radius {actual_radius:.3f}\" is below NEC minimum "
                f"{min_r:.3f}\" for {conduit_type.value} {trade_size.value}. "
                f"Deficit: {min_r - actual_radius:.3f}\"."
            ),
...
def calculate_developed_length(
    ...
    Args:
        bend_radius: Centreline bend radius in inches.
    ...
    if not math.isfinite(bend_radius) or bend_radius <= 0.0:
        return Result.err(PhysicsError(
            message=f"bend_radius={bend_radius} must be a positive finite number.",
    ...
    arc = math.pi * bend_radius * angle_deg / 180.0
