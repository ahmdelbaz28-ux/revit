...
def api_call(backend: str, admin_key: str, master_token: str, method: str, path: str, body: dict = None) -> dict:
        """Make an authenticated API call to the BAZSPARK backend (V240: requires master token)."""
        ...
        try:
                ...
                if e.code == 401:
                        ...
                        print("   → Forbidden: Master admin token required (X-Master-Admin-Token header).", file=sys.stderr)
                        print("   → Or: The API key does not have admin permissions.", file=sys.stderr)
                elif e.code == 429:
                        print("   → Rate limited (10 ops/minute). Wait 60 seconds and try again.", file=sys.stderr)
                sys.exit(1)
...
def generate_key(backend: str, admin_key: str, master_token: str, role: str, description: str) -> None:
        ...
        result = api_call(backend, admin_key, master_token, "POST", "/admin/keys", {
                "role": role,
...
def list_keys(backend: str, admin_key: str, master_token: str) -> None:
        """List all API keys."""
        print(f"\n📋 Listing all API keys from {backend}...")
        print()

        result = api_call(backend, admin_key, master_token, "GET", "/admin/keys")
...
def delete_key(backend: str, admin_key: str, master_token: str, key_hash: str) -> None:
        """Delete an API key by hash."""
        print(f"\n🗑️  Deleting API key {key_hash[:16]}...")
        print()

        result = api_call(backend, admin_key, master_token, "DELETE", f"/admin/keys/{key_hash}")
...
def main() -> None:
        parser = argparse.ArgumentParser(
                description="BAZSPARK API Key Generator (V240 — requires master token)",
        ...
        parser.add_argument(
                "--admin-key",
                required=True,
                help="The ADMIN API key (used for login + admin auth)",
        )
        parser.add_argument(
                "--master-token",
                required=True,
                help="The MASTER ADMIN TOKEN (separate from API key — required for key management)",
        )
        parser.add_argument(
                "--backend",
        ...
        if args.list:
                list_keys(args.backend, args.admin_key, args.master_token)
        elif args.roles:
                list_roles(args.backend, args.admin_key, args.master_token)
        elif args.delete:
                delete_key(args.backend, args.admin_key, args.master_token, args.delete)
        elif args.role:
                generate_key(args.backend, args.admin_key, args.master_token, args.role, args.description)
