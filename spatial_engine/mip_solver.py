...
class OptimalMIPEngine:
    ...
    def solve(self) -> Tuple[List[Tuple[float, float]], int, bool, Dict]:
        ...
        # V46 SAFETY: Progressive redundancy tiers — fire detection must not have
        # single points of failure. Even if 2 detectors mathematically cover a room,
        # 3 provides critical redundancy: if one detector fails (dust, maintenance
        # lag, wiring fault), the remaining detectors still provide coverage.
        # NFPA 72 §17.6.1 requires supervision but not count redundancy; however,
        # engineering best practice for life-safety systems demands redundancy.
        area = self.polygon.area
        min_detectors = 0
        if area >= 60.0:
            min_detectors = 1  # At least 1 for any viable room
        if area >= 100.0:
            min_detectors = 3  # Redundancy for medium rooms (e.g. 10×10m)
        elif area >= 80.0:
            min_detectors = 2  # Coverage + basic redundancy
        if area >= 200.0:
            min_detectors = max(min_detectors, 4)  # Full redundancy for large rooms
        if min_detectors > 0:
