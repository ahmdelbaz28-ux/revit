...
class UniversalDataModel:
    ...
    def __init__(self, db_path: str = "fireai_universal.db"):
        ...
        # V46 SAFETY: Tamper detection flag — if engine is explicitly set to
        # None (sabotage), subsequent operations must fail loudly, not silently
        # continue with potentially corrupted state. This is a safety-critical
        # system; silent failure after tampering could mask data corruption.
        self._engine_tampered = False
    ...
    # ── V46 SAFETY: engine property with tamper detection ────────────────
    # Provides backward compatibility with code that references .engine
    # (e.g., SQLAlchemy-based versions). Also detects intentional sabotage:
    # setting engine=None marks the database as tampered, causing all
    # subsequent operations to raise AttributeError — failing loudly is
    # safer than silently continuing with corrupted state.
    @property
    def engine(self):
        """Return the database connection (backward compat + tamper check)."""
        if self._engine_tampered:
            raise AttributeError(
                "Database engine has been tampered with (set to None). "
                "All operations are blocked for safety."
            )
        return self._conn
    ...
    def engine(self, value):
        if value is None:
            self._engine_tampered = True
            logger.critical(
                "SAFETY ALERT: Database engine set to None — "
                "tamper detection activated. All operations blocked."
            )
        else:
            self._engine_tampered = False
            self._conn = value
    ...
    def get_all_elements(self) -> List[UniversalElement]:
        ...
        V46 SAFETY: Raises AttributeError if engine has been tampered with."""
        if self._engine_tampered:
            raise AttributeError(
                "Database engine has been tampered with (set to None). "
                "All operations are blocked for safety."
