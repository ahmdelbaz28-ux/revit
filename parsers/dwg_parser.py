...
SAFETY-CRITICAL: Reads DWG via LibreDWG tools or DDC converter.

CONVERTER PRIORITY:
  1. DDC ddc-dwgconverter (preferred — produces structured XLSX with all BIM data)
     Install: curl -sS https://pkg.datadrivenconstruction.io/setup.sh | sudo bash
              sudo apt install ddc-dwgconverter
  2. LibreDWG dxf-out (fallback — converts to DXF, then DXFParser)
     Install: sudo apt install libredwg-tools
